...
function ProfilePanel({
  ...
  if (!profile || !profile.deterministic_built_at) {
    ...
    }, "No profile built for this contact yet. It appears after their next order or reply, or on the nightly sweep.");
  }
  const money = cents => typeof cents === 'number' ? `$${(cents / 100).toLocaleString(undefined, {
  ...
  // nobody measured.
  const cadenceNote = profile.reorder_interval_source === 'personal' ? `their own rhythm, ${profile.cadence_confidence} confidence` : 'shop average, not measured for them';
  const rows = [['Orders', profile.order_count ?? 0], ['Total spent', money(profile.total_spend_cents)], ['Average order', money(profile.avg_order_value_cents)], ['First order', profile.first_order_at ? relativeTime(profile.first_order_at) : '—'], ['Last order', profile.last_order_at ? relativeTime(profile.last_order_at) : '—'], ['Last product', profile.last_product_name || 'none we can name'], ['Buys about every', profile.reorder_interval_days ? `${profile.reorder_interval_days} days` : '—'], ['Due back', profile.reorder_due_at ? relativeTime(profile.reorder_due_at) : '—'], ['Engagement', TIERS[profile.engagement_tier] || profile.engagement_tier || '—'], ['Messages in / out', `${profile.inbound_message_count ?? 0} / ${profile.outbound_message_count ?? 0}`], ['Campaigns received', profile.campaigns_received_count ?? 0]];
  return /*#__PURE__*/React.createElement(React.Fragment, null, profile.has_only_unpaid_orders && /*#__PURE__*/React.createElement("div", {
    className: "intel-section",
    style: {
      color: 'var(--warning, #d97706)'
    }
  }, "Every order this contact placed was cancelled or failed. They have never actually bought anything."), /*#__PURE__*/React.createElement("div", {
    className: "intel-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "intel-label"
  }, "Profile"), rows.map(([label, value]) => /*#__PURE__*/React.createElement("div", {
    key: label,
  ...
  }, cadenceNote)), profile.top_skus?.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "intel-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "intel-label"
  }, "Comes back for"), /*#__PURE__*/React.createElement("div", null, profile.top_skus.map(sku => /*#__PURE__*/React.createElement("span", {
    key: sku,
    className: "tag-chip green"
  }, sku)))), /*#__PURE__*/React.createElement("div", {
    className: "profile-note"
  }, "Measured from orders and messages, not written by a model. Built ", relativeTime(profile.deterministic_built_at), "."));
}
function OrderCard({
  order
}) {
...
function ContactModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    key: order.id,
    order: order
  }))), tab === 'profile' && /*#__PURE__*/React.createElement(ProfilePanel, {
    profile: intel
