...
function VoiceTab({
  ...
  }), voiceReady ? 'Connected' : 'Not connected', !voiceReady && /*#__PURE__*/React.createElement("button", {
  ...
  }, "Reconnect"))), activeSection === 'dialer' && /*#__PURE__*/React.createElement(DialerSection, {
  ...
  useEffect(() => {
    ...
    if (params.get('call') === 'incoming') {
      ...
      if (voiceReady) {
        ...
        api('POST', '/api/voice/sip-ready').then(d => console.log('[VOICE] Already-connected sip-ready ping:', d.status)).catch(() => {});
      } else {
        initVoiceClient();
    ...
    try {
      ...
      client.on('telnyx.ready', () => {
        ...
        if (openedForCall.current) {
          openedForCall.current = false;
          setTimeout(() => {
            api('POST', '/api/voice/sip-ready').then(d => console.log('[VOICE] SIP-ready ping:', d.status)).catch(() => {});
          }, 1500);
        }
      });
