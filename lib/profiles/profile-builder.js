 ...
 */
// 2: added attempted_product_*, which is null for every version-1 row because
// the columns did not exist when they were written. The fingerprints could not
// see this — they answer "have this contact's rows changed", and nothing about
// any contact changed. This is precisely the case the version exists for.
...
const COLUMNS = Object.freeze([
  ...
  'attempted_product_name', 'attempted_product_sku', 'attempted_order_at', 'attempted_value_cents',
...
function computeProfile({
  ...
  // ── WHAT THEY TRIED TO BUY, WHEN THEY NEVER BOUGHT ───────────────────────
  //
  // Only for somebody with NO paid order. 125 contacts are in that position,
  // holding $25,009.88 of orders that never completed and 71 attempts at RT,
  // and every column describing a product is empty for them by design — so a
  // campaign to them cannot name the one thing worth naming.
  //
  // Kept separate from last_product_name rather than filled into it. That
  // column means "what they bought", and a message naming a product somebody
  // never received is a lie they can catch: twelve people were told "saw you
  // ordered RT in May" about a cancelled May order. Two questions, two
  // answers, both labelled, and nothing that means "bought" reads from this.
  const unpaidOrders = (orders || [])
    .filter(order => !PAID_STATUSES.has(String(order?.status || '').toLowerCase()))
    .sort((a, b) => Date.parse(b?.created_at || 0) - Date.parse(a?.created_at || 0));
  const newestAttempt = paid.length === 0 ? unpaidOrders[0] || null : null;
  const attempted = newestAttempt
    ? approvedLabelForOrder(newestAttempt, skuMap)
    : { label: null, sku: null };
  const attemptedValueCents = newestAttempt
    ? Math.round(Number(newestAttempt.total || 0) * 100) || 0
    : 0;
  ...
  const payload = {
    ...
    attempted_product_name: attempted.label,
    attempted_product_sku: attempted.sku ? normaliseSku(attempted.sku) : null,
    attempted_order_at: newestAttempt ? isoOrNull(Date.parse(newestAttempt.created_at)) : null,
    attempted_value_cents: attemptedValueCents,
...
async function profileablePhones({ client }) {
  // ── EVERYBODY WHO ORDERED, NOT EVERYBODY WHO PAID ────────────────────
  //
  // This filtered to paid statuses, so the 123 people whose orders ALL failed
  // or cancelled never got a profile row at all — and has_only_unpaid_orders,
  // the column that exists precisely to stop them being treated as returning
  // customers, could never be true for anybody. A guard that cannot reach the
  // people it guards is not a guard.
  //
  // Measured: 125 contacts have only unpaid orders, $25,009.88 of orders that
  // never completed, 71 of them for RT. Two share an email with a paying
  // customer and none shares a phone, so they are not duplicates of existing
  // buyers: they are people who tried to buy and did not. That is a segment
  // somebody can act on, and it was invisible.
...
module.exports = {
  ...
  fingerprintOf,
  profileablePhones,
  // Exported so the narrative writer reads through the SAME chunked-AND-paged
  // helper rather than growing a third copy of a pattern that has taken
  // production down twice. See the file header.
  readChunked,
  refreshProfile,
