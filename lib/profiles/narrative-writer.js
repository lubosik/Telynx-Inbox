...
function narrativeGate({ profile, now = new Date(), env = process.env } = {}) {
  ...
  // The cooldown is the cost ceiling, and it only applies to a REBUILD. A
  // contact who has never had a narrative is built immediately: waiting seven
  // days to say anything about somebody who has been talking to us is not
  // thrift, it is just lateness.
...
const SYSTEM_PROMPT = [
  ...
  '',
  `THE SHOP: ${BUSINESS.name}, ${BUSINESS.what}. ${BUSINESS.customers}.`,
  '',
