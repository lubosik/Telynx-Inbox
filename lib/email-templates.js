...
/**
 * The promotional SMS opt-in invitation.
 *
 * THIS ONE GOES TO A CUSTOMER, NOT TO STAFF
 *   Every other template in this file is addressed to somebody who works here.
 *   This is the only one sent to a member of the public, under the marketing
 *   EMAIL permission Vici actually holds, in order to ask for the SMS
 *   permission it does not. Everything about the tone follows from that: it is
 *   asking a favour, so it is short, it is plain, and it does not shout.
 *
 * NO OFFER, NO INCENTIVE, NO URGENCY
 *   Deliberately. A discount attached to the ask makes the resulting consent
 *   worth less, not more: "consent is not a condition of purchase" is a much
 *   weaker sentence when the message it sits in is buying the answer. There is
 *   no countdown, no "last chance", and no pre-ticked anything. The link goes
 *   to a page with two real buttons on it.
 *
 * THE DISCLOSURE IS IN THE EMAIL AS WELL AS ON THE PAGE
 *   Frequency, cost, and how to stop appear here too, because a person deciding
 *   whether to open the link should not have to open it to find out what they
 *   would be agreeing to. The frequency numbers are the caps
 *   lib/campaigns/cadence-policy.js enforces (2 in 7 days, 4 in 30). If that
 *   policy changes, this copy changes with it.
 *
 * THE NUMBER IS NAMED, BY ITS LAST FOUR DIGITS
 *   "We would text the number ending 0132" is the one fact the page cannot
 *   safely show — the page is reachable by anybody holding a forwarded link,
 *   while this mailbox belongs to the person being asked. Without it, somebody
 *   with two numbers on file cannot tell what they are agreeing to.
 *
 * NO TRACKING
 *   No pixel, no redirect wrapper, no per-recipient beacon, exactly as with
 *   every other message in this file. Whether an opt-in email was opened is not
 *   information worth putting a scanner-triggerable URL into a consent flow for.
 *
 * @param {object} invite
 * @param {string} [invite.recipientName]
 * @param {string} invite.optInUrl        ${APP_URL}/sms-optin?token=...
 * @param {string} [invite.phoneEnding]   last four digits, from phoneEnding()
 * @param {string} [invite.expiresAt]     ISO timestamp
 * @param {number} [invite.expiryDays]
 * @param {string} [invite.brandName]     the customer-facing brand, not the app
 * @returns {{subject: string, text: string, html: string}}
 */
function smsOptInInviteEmail(invite = {}) {
  const brandName = String(invite.brandName || '').trim() || 'Vici Peptides';
  const optInUrl = String(invite.optInUrl || '').trim();
  const ending = String(invite.phoneEnding || '').trim();
  const days = Number.isFinite(Number(invite.expiryDays)) && Number(invite.expiryDays) > 0
    ? Math.round(Number(invite.expiryDays))
    : 30;
  const expiry = formatExpiry(invite.expiresAt);
  const greeting = greetingFor(invite.recipientName);

  const subject = `Can we text you? ${brandName} needs your permission first`;

  const openingLine = `You are on the ${brandName} email list, which is how this message reached you. `
    + 'We have never had your permission to send text messages, and we are not going to assume it. '
    + 'So we are asking.';

  const whatLine = ending
    ? `If you say yes, we would text the number ending ${ending} when something you buy comes back `
      + 'in stock, when you are likely due a reorder, and occasionally with an offer.'
    : 'If you say yes, we would text you when something you buy comes back in stock, when you are '
      + 'likely due a reorder, and occasionally with an offer.';

  const limitsLine = 'Never more than 2 messages in any 7 days, and never more than 4 in any 30 days. '
    + 'That is a limit our system enforces rather than an intention. Message and data rates may apply, '
    + 'and you can end them at any time by replying STOP to any text.';

  const noPressureLine = 'This is not a condition of buying anything. Saying no changes nothing about '
    + 'your account, your orders, your prices or your support, and your order and delivery updates are '
    + 'separate from this either way.';

  const expiryLine = expiry
    ? `The link works for ${days} days, until ${expiry}. There is a "no thanks" button on the page as well, `
      + 'and pressing it saves us both from asking again.'
    : `The link works for ${days} days. There is a "no thanks" button on the page as well, and pressing it `
      + 'saves us both from asking again.';

  const ignoreLine = 'If you would rather not deal with this at all, ignore this email. '
    + 'Nothing happens, and you will not be sent marketing texts.';
  ...
  const safe = {
    title: escapeHtml(brandName),
    greeting: escapeHtml(greeting),
    opening: escapeHtml(openingLine),
    what: escapeHtml(whatLine),
    limits: escapeHtml(limitsLine),
    noPressure: escapeHtml(noPressureLine),
    expiry: escapeHtml(expiryLine),
    ignore: escapeHtml(ignoreLine),
    // Escaped for the href as well as the visible text. The token is base64url
    // and cannot contain a quote, but the URL is never interpolated raw.
    url: escapeHtml(optInUrl)
  };
  ...
  const html = cardShell({
    ...
    safeParagraphs: [
      ...
      `<a href="${safe.url}" style="display:inline-block;background-color:#18181b;color:#ffffff;`
        + 'font-family:Helvetica,Arial,sans-serif;font-size:16px;line-height:20px;font-weight:bold;'
        + 'text-decoration:none;padding:14px 24px;border-radius:6px;">Choose yes or no</a>',
      'If the button does not work, copy and paste this link into your browser:<br>'
        + `<a href="${safe.url}" style="color:#3f3f46;word-break:break-all;">${safe.url}</a>`,
      safe.expiry
    ],
