...
const METADATA_ALLOWLIST = Object.freeze({
  ...
  // Internal referral notes are intentionally absent. `note_present` records
  // that context existed without copying customer-adjacent text into the
  // immutable audit table.
  'conversation.referral.created': ['referral_id', 'target_kind', 'target_user_id', 'owner_user_id', 'state', 'note_present'],
  'conversation.referral.claimed': ['referral_id', 'target_kind', 'target_user_id', 'owner_user_id', 'state', 'note_present'],
  'conversation.referral.reassigned': ['referral_id', 'target_kind', 'target_user_id', 'owner_user_id', 'state', 'note_present'],
  'conversation.referral.handed_back': ['referral_id', 'target_kind', 'target_user_id', 'owner_user_id', 'state', 'note_present'],
  'conversation.referral.resolved': ['referral_id', 'target_kind', 'target_user_id', 'owner_user_id', 'state', 'note_present'],
  ...
  'campaign.created': ['campaign_type', 'workflow_category', 'recipient_count', 'message_length', 'message_digest'],
  'campaign.drafts.generated': [
    'mode', 'workflows', 'opportunities_prepared', 'drafts_prepared',
    'campaigns_inserted', 'campaigns_reused', 'notifications_targeted',
    'notifications_sent', 'notifications_disabled'
  ],
  'campaign.edited': ['revision', 'message_length', 'message_digest'],
  'campaign.review_submitted': ['revision', 'recipient_count'],
  'campaign.rejected': ['revision', 'reason'],
  'campaign.approved': ['revision', 'recipient_count', 'audience_digest', 'message_length', 'message_digest'],
  'campaign.scheduled': ['revision', 'scheduled_for'],
  'campaign.rescheduled': [
    'revision', 'previous_scheduled_for', 'scheduled_for', 'business_timezone'
  ],
  'campaign.launched': [],
  'campaign.cancelled': ['revision', 'reason'],
  // No raw phone and no message body. The last four digits are enough to
  // distinguish which of the owner's handsets received the test.
  'campaign.test_sent': ['target_last4', 'segments', 'characters', 'provider_status'],

  // A proposal decision. `opportunity_id`, `mechanism` and `segment_key` are
  // machine keys for a cohort, never a person; the cohort label and the
  // message text are deliberately absent, and `message_digest` carries the
  // same sha256-and-length treatment every other campaign row uses.
  'campaign.proposal.accepted': [
    'proposal_id', 'opportunity_id', 'opportunity_kind', 'mechanism', 'offer_kind',
    'segment_key', 'campaign_id', 'recipient_count', 'message_length', 'message_digest'
  ],
  'campaign.proposal.dismissed': [
    'proposal_id', 'opportunity_id', 'opportunity_kind', 'mechanism', 'offer_kind', 'reason'
  ]
});
