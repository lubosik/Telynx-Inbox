...
const EVENT_TYPES = Object.freeze({
  ...
  // Notes stay in the referral event ledger and never enter immutable audit
  // metadata. These rows record only the named actor and state transition.
  'conversation.referral.created': Object.freeze({
    category: 'messages', entityType: 'conversation_referral', visibility: 'feed', severity: 'notice'
  }),
  'conversation.referral.claimed': Object.freeze({
    category: 'messages', entityType: 'conversation_referral', visibility: 'feed', severity: 'notice'
  }),
  'conversation.referral.reassigned': Object.freeze({
    category: 'messages', entityType: 'conversation_referral', visibility: 'feed', severity: 'notice'
  }),
  'conversation.referral.handed_back': Object.freeze({
    category: 'messages', entityType: 'conversation_referral', visibility: 'feed', severity: 'notice'
  }),
  'conversation.referral.resolved': Object.freeze({
    category: 'messages', entityType: 'conversation_referral', visibility: 'feed', severity: 'info'
  }),
  ...
  'campaign.coupon_created': Object.freeze({
    category: 'campaigns', entityType: 'campaign_coupon', visibility: 'feed', severity: 'notice'
  }),
  ...
  // Turning the automatic 21-day check-in on or off. consentBearing, because
  // switching it ON is a standing authorisation to message customers with no
  // human reading the campaign first — the same substance as approving one,
  // spread over every future sweep. 'warning' so it stands out in the feed:
  // this is the row somebody will want to find when they ask why a check-in
  // went out that nobody remembers approving.
  'campaign.settings_changed': Object.freeze({
    category: 'campaigns', entityType: 'campaign', visibility: 'feed', severity: 'warning', consentBearing: true
  }),
  'campaign.scheduled': Object.freeze({
    category: 'campaigns', entityType: 'campaign', visibility: 'feed', severity: 'notice'
  }),
  'campaign.rescheduled': Object.freeze({
    category: 'campaigns', entityType: 'campaign', visibility: 'feed', severity: 'notice'
  }),
  ...
  // The inverse. Not consent-bearing: putting a campaign back on the working
  // list sends nothing and authorises nothing, it only makes a row visible
  // again. Recorded all the same, so "who un-archived this" has an answer.
  'campaign.unarchived': Object.freeze({
    category: 'campaigns', entityType: 'campaign', visibility: 'feed', severity: 'info'
  }),
  // A real message to a real handset, so it is recorded like one. Not
  // consent-bearing: this is a one-off, human-directed test destination and
  // never a campaign audience member or an automated delivery.
  'campaign.test_sent': Object.freeze({
    category: 'campaigns', entityType: 'campaign', visibility: 'feed', severity: 'notice'
  }),
  // Who was taken out of an audience, and by whom. A campaign that silently
  // shrinks cannot be audited afterwards.
  'campaign.recipient_removed': Object.freeze({
    category: 'campaigns', entityType: 'campaign', visibility: 'feed', severity: 'info'
  }),
  ...
  // Destroying a segment row is irreversible, so it is a warning and it is on
  // the feed. It is only ever reachable for a segment that no campaign used,
  // that the engine never ran on, that carries no override and where nobody
  // wrote down why a named person is in it. Everything else archives. See
  // delete_sms_campaign_segment in scripts/segment-lifecycle-migration.sql,
  // which enforces that in SQL rather than trusting the route.
  // Deciding a business will never message somebody again, and reversing that
  // decision. Both are on the feed: a list of blocked people is evidence, and
  // evidence that can change without a trace is not evidence.
  'campaign.suppression.added': Object.freeze({
    category: 'campaigns', entityType: 'contact', visibility: 'feed', severity: 'notice'
  }),
  'campaign.suppression.removed': Object.freeze({
    category: 'campaigns', entityType: 'contact', visibility: 'feed', severity: 'warning'
  }),
  'campaign.segment.deleted': Object.freeze({
    category: 'campaigns', entityType: 'campaign_segment', visibility: 'feed', severity: 'warning'
  }),
  // Reversible, and the row survives. It is how a segment that carries a record
  // of a decision leaves the working list without any of it being destroyed.
  'campaign.segment.archived': Object.freeze({
    category: 'campaigns', entityType: 'campaign_segment', visibility: 'feed', severity: 'notice'
  }),
  'campaign.segment.restored': Object.freeze({
    category: 'campaigns', entityType: 'campaign_segment', visibility: 'feed', severity: 'notice'
  }),

  // ── Campaign proposals ───────────────────────────────────────────────────
  // A proposal is a draft of an argument, generated from a detected cohort
  // opportunity. Generating one records nothing: it writes no campaign, has no
  // recipients and changes nobody's state, so there is no event type for it.
  //
  // The two DECISIONS are recorded, because both are a named person deciding
  // something about customer-facing work.
  //
  // Accepting is on the feed and is a notice: it produces an ordinary campaign
  // draft, and "where did this campaign come from" must have an answer that
  // does not depend on the proposal row still existing.
  'campaign.proposal.accepted': Object.freeze({
    category: 'campaigns', entityType: 'campaign_proposal', visibility: 'feed', severity: 'notice'
  }),
  // Dismissal is detail rather than feed — it will be the common outcome — but
  // it is recorded with its reason, because
  // docs/campaigns/TRACKING-AND-LEARNING-RESEARCH.md is explicit that the
  // counterfactual is the part that tells you whether the loop is improving or
  // whether the human is doing all the work.
  'campaign.proposal.dismissed': Object.freeze({
    category: 'campaigns', entityType: 'campaign_proposal', visibility: 'detail', severity: 'info'
  })
});
