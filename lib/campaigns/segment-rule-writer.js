...
/**
 * Vici terms whose business meaning is fixed rather than inferred.
 *
 * These are intentionally ordinary rules from the closed grammar. They gain
 * no privileged evaluator or query path: the only special treatment is that
 * the phrase itself has one stable meaning. The operator still sees the two
 * rules, the real matching count and customer samples before Save is enabled.
 */
const VICI_PERMANENT_AUDIENCE_DEFINITIONS = Object.freeze({
  best_repeat_customers: Object.freeze({
    key: 'best_repeat_customers',
    label: 'Best Repeat Customers',
    meaning: 'customers with at least 3 paid orders and at least 500 dollars in lifetime spend',
    rules: Object.freeze({
      match: 'all',
      conditions: Object.freeze([
        Object.freeze({ dimension: 'order_count', operator: 'at_least', value: 3 }),
        Object.freeze({ dimension: 'lifetime_spend', operator: 'at_least', value: 500 })
      ])
    })
  })
});

const BEST_REPEAT_ALIASES = new Set([
  'best repeat customer',
  'best repeat customers',
  'our best repeat customer',
  'our best repeat customers',
  'best and most repeat customer',
  'best and most repeat customers',
  'our best and most repeat customer',
  'our best and most repeat customers',
  'best customers and repeat customers',
  'our best customers and repeat customers',
  'best customers and our repeat customers',
  'our best customers and our repeat customers'
]);

/**
 * Remove only common voice-command framing. The remainder must equal one of
 * the fixed aliases. A request with an additional product, date or other
 * condition deliberately does not match here; the translator must preserve
 * that extra meaning rather than this shortcut silently dropping it.
 */
function permanentAudienceDefinition(rawDescription) {
  ...
  text = text.replace(/^(?:i\s+(?:want|wanna|would like)\s+to|can you)\s+/, '');
  text = text.replace(/^(?:create|build|make|do)\s+(?:me\s+)?(?:a\s+)?(?:segment|audience|group)\s+(?:with|for|of|made up of)\s+/, '');
  text = text.replace(/^(?:show me|target)\s+/, '');
  text = text.replace(/^(?:a\s+)?(?:segment|audience|group)\s+(?:with|for|of|made up of)\s+/, '');
  text = text.trim();

  return BEST_REPEAT_ALIASES.has(text)
    ? VICI_PERMANENT_AUDIENCE_DEFINITIONS.best_repeat_customers
    : null;
}
 ...
 *
 * `$` is deliberately NOT here. It is the one of these an operator has a real
 * reason to type, `${` is already covered by the brace, and a currency amount
 * gets its own refusal below that tells them how to write it instead. A glyph
 * error on "spent more than $500" would be technically correct and useless.
 */
...
function buildSystemPrompt() {
  return [
    ...
    '11. VICI PERMANENT BUSINESS DEFINITION: "best repeat customers", "best and most',
    '    repeat customers", and equivalent wording mean customers with BOTH order_count',
    '    at_least 3 AND lifetime_spend at_least 500. This term is not ambiguous. Do not',
    '    ask what "best" means. If the request adds other conditions, retain them as well.',
    '',
...
async function draftRulesFromDescription(input = {}, dependencies = {}) {
  ...
  // A named Vici definition is deterministic. It still passes through the
  // same validator and the caller still has to preview the real audience
  // before saving. Avoiding a model call here prevents the agreed threshold
  // from changing because of wording, provider availability or model drift.
  const permanent = permanentAudienceDefinition(description);
  if (permanent) {
    const verdict = validate(permanent.rules, {
      products,
      segments,
    ...
    if (!verdict.ok) {
      throw new SegmentRuleDraftError(
        'The permanent Best Repeat Customers definition did not pass the segment safety checks.',
        'SEGMENT_PERMANENT_DEFINITION_INVALID', 500
      );
    }
    return {
      status: 'drafted',
      description,
      model: null,
      permanentDefinition: permanent.key,
