...
async function prepareOpportunityDraftRun(input = {}, dependencies = {}) {
  // A SEGMENTATION INPUT IS NOT A SEND PATH.
  //
  // buildSegmentationInput() deliberately builds candidates for people who
  // have NO current commercial clearance, so that a segment can answer "who
  // matches this pattern" on real data instead of showing an empty screen.
  // Those candidates carry `commercialClearance.clear === false` and the whole
  // input is stamped `segmentationOnly`. Feeding one of them to the draft
  // preparer would silently convert a widened VIEW into a widened SEND, which
  // is the exact mistake this stamp exists to make impossible.
  //
  // This is a throw, not a filter. Filtering would let the wrong call site
  // keep working and hide the mistake; the throw names it at the boundary.
  if (input?.segmentationOnly === true || input?.clearanceMode === 'observe') {
    throw Object.assign(
      new Error('A segmentation input carries people with no commercial clearance and can never be drafted or sent from.'),
      { code: 'SEGMENTATION_INPUT_IS_NOT_A_SEND_PATH' }
    );
  }
