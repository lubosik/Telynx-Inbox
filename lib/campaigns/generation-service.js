...
/**
 * The most recent of a list of raw order timestamps, as an ISO string.
 *
 * Orders are pushed in whatever order the source rows arrived, so this takes
 * the maximum rather than the last element. An unparseable value is skipped
 * rather than poisoning the answer, and no readable value at all returns null,
 * which every reader treats as "we do not know" rather than as "never".
 */
function latestPurchaseISO(timestamps) {
  let latest = null;
...
function buildGenerationInput(sources, {
  ...
  if (enabled.has('back_in_stock')) {
    for (const event of sources.restockEvents || []) {
      ...
      if (!buyers) continue;
      // Is the thing that came back still in stock NOW? Read once per event
      // from the same merged inventory the reorder half uses, with the same 24
      // hour freshness rule, so an item that returned on Monday and went out
      // again on Wednesday reads false rather than reading as good news.
      // Additive and observational: nothing that existed before this line
      // reads it, and the send path performs its own authoritative re-fetch
      // regardless.
      const currentlyInStock = currentInventory(
        inventoryFor(inventory, Number(event.product_id), Number(event.variation_id || 0)), at
      );
      for (const phone of buyers.keys()) {
        ...
        // Every time this person bought THE ITEM THAT CAME BACK, at the level
        // the event was reported at. Not the parent-product reorder series:
        // that is a different question, it is answered by the reorder
        // candidate, and it is deliberately not what a stock claim rests on.
        //
        // A restock segment needs these two numbers and has no other way to
        // reach them. Somebody whose buying is unreadable has no reorder
        // candidate at all, so "when did they last buy the thing that came
        // back" would otherwise be unanswerable for exactly the people a
        // cadence-free restock list is made of.
        const buyerPurchases = buyers.get(phone) || [];
        backInStockCandidates.push({
          ...
          lastPurchaseAt: latestPurchaseISO(buyerPurchases),
          repeatBuyer: buyerPurchases.length > 1,
