...
function normaliseOpportunity(raw) {
  ...
  // ── THIS FUNCTION'S OUTPUT MUST BE VALID AS ITS OWN INPUT ──────────────
  //
  // It DERIVES `contractVersion` and `kindLabel` and adds them to what it
  // returns (see the return below), and its input list used to forbid both. So
  // feeding its own output back in failed with "Unexpected opportunity field:
  // contractVersion, kindLabel" — and opportunityFromFinding, its sibling in
  // this same file, returns exactly that shape.
  //
  // The cost was the whole automatic proposal feature. The daily cycle adapted
  // each finding and handed it to the drafter, which re-validated it here and
  // threw. Measured in production: six findings offered every day for six days
  // running, `wouldDraft: 6, drafted: 0, saved: 0`, every one of them
  // OPPORTUNITY_CONTRACT_REJECTED — with the enabling flag switched ON the
  // whole time. It looked like a disabled feature and was a broken one.
  //
  // routes/assistant.js had already hit this and worked around it locally, with
  // a comment calling it "a sharp edge worth absorbing here once rather than at
  // each of the two call sites". It was absorbed at one of the two. The other
  // was the automatic path, so nothing was produced and nobody was told.
  //
  // Both are ACCEPTED and neither is TRUSTED: the values below are recomputed
  // from `kind` and from this module's own constant regardless of what arrives,
  // so a caller cannot use them to smuggle text into a prompt.
  const allowed = new Set([
    'id', 'kind', 'title', 'cohort', 'facts', 'sizing', 'detectedAt', 'detectorVersion',
    'contractVersion', 'kindLabel'
  ]);
