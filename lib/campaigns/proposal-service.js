...
function createProposalService({
  ...
  async function saveBatch(proposals, { model = null } = {}) {
    ...
    // ── NEW IS NOT THE SAME AS SAVED ───────────────────────────────────────
    //
    // A proposal still sitting in `proposed` is REFRESHED by the upsert below,
    // which is right: its numbers should track the cohort. But it is not news.
    //
    // The daily digest asks this service how many proposals were drafted and
    // pushes a notification when the answer is above zero, bypassing its own
    // novelty gate on the reasoning that "a new proposal is news on its own".
    // That reasoning holds only if "drafted" means NEW. Counting refreshes as
    // drafts would push "6 campaign proposals are waiting for review" every
    // single morning, about the same six proposals, forever — which is exactly
    // the notification nobody reads by the third day.
    //
    // So the keys that already existed in ANY state are recorded here, and
    // `created` counts only the genuinely new ones.
    const alreadyPresent = new Set((existingRows || []).map(row => row.proposal_key));
    ...
    for (const proposal of list) {
      ...
      if (!alreadyPresent.has(proposal.proposalKey)) created.push(proposal.proposalKey);
      writable.push(toRow(proposal, workspaceID, model));
    }
    if (!writable.length) return { saved: [], skipped, created: [] };
    ...
    return { saved: (data || []).map(shapeProposal), skipped, created };
  }
  ...
  async function resolveSegmentRecipients(segmentKey) {
    ...
    for (let from = 0; from < MAX_SEGMENT_MEMBER_ROWS; from += DB_PAGE_SIZE) {
      const { data, error } = await db().from('sms_campaign_segment_members')
        // inclusion_evidence is the per-person "why". buyer-cohorts.js writes a
        // real answer into it — order count, when their only order was, what
        // it was worth, whether what they bought is still on sale, and the
        // tenure basis the rate was drawn from. Selecting only the three
        // identity columns threw all of that away at exactly the moment it
        // became useful, so an accepted campaign's recipient list could say
        // nothing beyond "from segment X".
        .select('contact_phone,contact_id,contact_name_snapshot,inclusion_evidence')
    ...
    return {
      ...
      recipients: rows.map(row => ({
        ...
        // Carried through, not summarised. This lands in
        // sms_campaign_recipients.inclusion_reason, which the app already
        // renders, so "why is this person in this campaign" is answerable on
        // the campaign itself rather than only on the segment screen.
        reason: {
          source: 'campaign_proposal',
          segment_key: segmentKey,
          evidence: row.inclusion_evidence ?? null
        }
      }))
    };
  }
