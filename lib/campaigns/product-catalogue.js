 ...
 */
async function fetchProductCatalogue(wooGet) {
...
async function productCatalogue({ wooGet, env = process.env, now = new Date(), forceRefresh = false } = {}) {
  ...
  if (!inflight) {
    inflight = (async () => {
      try {
        const snapshot = await fetchProductCatalogue(read);
 ...
 */
function catalogueInventoryRows(snapshot, { workspaceID = 'vici' } = {}) {
  const observedAt = snapshot?.fetchedAt && Number.isFinite(Date.parse(snapshot.fetchedAt))
