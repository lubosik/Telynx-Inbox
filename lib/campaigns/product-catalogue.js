...
function catalogueEntries(product, variations = []) {
  ...
  // The WordPress shortlink rather than the readable slug. Fixed at 31 or 32
  // characters whatever the product is named, where a permalink runs to 58 and
  // grows with the product name — so the longest product names produced the
  // longest URLs, and a message could not hold a greeting, a name, a product,
  // a month, an offer and a link at once. It 301-redirects to the canonical
  // page, verified against the live store.
  let permalink = '';
  if (rawPermalink) {
    try {
      const origin = new URL(rawPermalink).origin;
      permalink = `${origin}/?p=${productID}`;
    } catch { permalink = rawPermalink; }
  }
