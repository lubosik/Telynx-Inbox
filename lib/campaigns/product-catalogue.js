...
function catalogueEntries(product, variations = []) {
  ...
  const rawPermalink = typeof product?.permalink === 'string' ? product.permalink.trim() : '';
  // ── THE READABLE PERMALINK, NOT THE SHORTLINK ──────────────────────────
  //
  // This rewrote every link to `?p=551`, on the reasoning that a shortlink is
  // a fixed 31 characters where a permalink runs to 58 and grows with the
  // product name, so a message could not hold a greeting, a name, a product,
  // a month, an offer and a link at once.
  //
  // The length was right and the conclusion no longer is. Measured at absolute
  // worst case — longest name, longest product label, longest code, longest
  // link — the win-back lands at 236 septets and the recovery campaign at 264,
  // against a 306 ceiling. The budget was never as tight as the shortlink was
  // buying its way out of.
  //
  // And the cost was never only characters. `vicipeptides.com/?p=551` in a
  // text message tells the reader nothing about where it goes, which for a
  // peptide shop asking somebody to click is the shape of every scam text they
  // have ever had. `vicipeptides.com/product/rt/` names the destination and
  // the product in the URL itself.
  //
  // Measured on the live catalogue: 23 published products, permalinks 36 to 58
  // characters, median 42.
