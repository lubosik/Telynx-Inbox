...
function buildUserPrompt({
  workflowType, productName, brandName, cadenceNote, brief, candidateCount, linkUrl, currentMessage, styleTraits, couponCode, minimumSpend
}) {
  const lines = [];
...
async function draftCandidates(input = {}, dependencies = {}) {
  ...
  const allowedKeys = new Set(['workflowType', 'productName', 'brandName', 'cadence', 'brief',
    'candidateCount', 'linkUrl', 'approvedProductCodes', 'currentMessage', 'styleTraits', 'couponCode', 'minimumSpend']);
  ...
  const messages = [
    ...
    {
      role: 'user',
      content: buildUserPrompt({ workflowType, productName, brandName, cadenceNote, brief, candidateCount: requested, linkUrl, currentMessage, styleTraits: input.styleTraits, couponCode, minimumSpend })
    }
  ];
