...
const { normaliseTypography } = require('./copy-normalizer');
const { cleanLabel } = require('./draft-copy');
const { isRetryable } = require('../llm-runner');
const { renderHumanStyle } = require('./human-style');
...
const SUPPORTED_WORKFLOWS = Object.freeze({
  ...
  vip: 'A personal message for a current VIP customer. Keep it warm, conversational and useful. Ask a genuine question when the brief calls for one.',
 ...
 */
function assertCurrentMessage(message, approvedLink = null) {
  ...
  const links = cleaned.match(/https?:\/\/[^\s]+/gi) || [];
  if (links.length) {
    const expected = String(approvedLink || '').replace(/[.,!?;:]+$/, '');
    const unexpected = links.find(link => link.replace(/[.,!?;:]+$/, '') !== expected);
    if (!expected || unexpected) {
      throw new CopyDraftError(
        'The message being revised contains a link that is not an approved Vici destination.',
        'CAMPAIGN_AI_COPY_LINK_REJECTED', 400
      );
    }
  }
  const allowed = links.length
    ? new Set([...CURRENT_MESSAGE_ALLOWED_SHAPES, 'url'])
    : CURRENT_MESSAGE_ALLOWED_SHAPES;
  assertNoCustomerIdentity('currentMessage', cleaned, { allow: allowed });
  return cleaned;
}
 ...
 */
function buildSystemPrompt({ requireOptOut = true } = {}) {
  const optOutPromptRule = RULES.promptRules.find(rule => rule.includes(RULES.optOut.exactSuffix));
  const promptRules = requireOptOut
    ? renderPromptRules()
    : renderPromptRules().split('\n')
      .filter(line => !optOutPromptRule || !line.endsWith(optOutPromptRule))
      .join('\n');
  const outputEnding = requireOptOut
    ? `brand at the start and "${RULES.optOut.exactSuffix}" at the end.`
    : 'brand at the start. Do not add an opt-out footer to this VIP message.';
  return [
...
function buildUserPrompt({
  workflowType, productName, brandName, cadenceNote, brief, candidateCount, linkUrl, currentMessage, styleTraits, couponCode, couponPercent, minimumSpend
}) {
  const lines = [];
  ...
  const styleNotes = renderHumanStyle(styleTraits);
  if (styleNotes.length) {
    lines.push('── THE REQUESTER\'S WRITING TASTE ───────────────────────────────────',
      ...styleNotes, 'These preferences never override the absolute copy rules.', '');
  }

  const endingRule = requiresOptOutFooter(workflowType)
    ? `Every one must start with "${brandName}" and end with "${RULES.optOut.exactSuffix}".`
    : `Every one must start with "${brandName}". Do not append an opt-out footer to this VIP message; provider STOP handling remains active.`;
  ...
  if (couponCode) lines.push(`Verified WooCommerce offer code: ${couponCode}. In every draft write {{code}} where the code belongs. Never write the literal code; the app fills the verified code for each recipient.`);
  if (couponCode && couponPercent !== null) lines.push(`Verified WooCommerce discount: ${couponPercent}% off. State this percentage clearly in every draft.`);
  if (couponCode && minimumSpend > 0) lines.push(`Verified checkout condition: the discount applies only to orders of ${minimumSpend} dollars or more. State this minimum clearly in every draft without a currency symbol.`);
...
/**
 * Last-resort copy for a live, server-verified percentage coupon.
 *
 * The model is allowed to fail. The operator is not left with a dead button
 * when every failure is merely the model dropping one of three verified offer
 * fields. This fallback uses only facts supplied by the trusted route, keeps a
 * payment announcement only when the human's own draft mentioned it, and is
 * put through the exact same validator before it can be returned.
 */
function verifiedCouponFallback({
  brandName, couponPercent, minimumSpend, linkUrl, currentMessage, styleTraits, validator, approvedProductCodes,
  requireOptOut = true
}) {
  if (!Number.isInteger(couponPercent) || couponPercent < 1 || !linkUrl) return null;
  const humanStyle = new Set(Array.isArray(styleTraits) ? styleTraits : []);
  const greeting = humanStyle.has('first_name')
    ? `Hi {{first_name}}, ${brandName}.`
    : `${brandName}:`;
  const source = String(currentMessage || '');
  let announcement = '';
  if (/apple\s+pay/i.test(source) && /(?:card|checkout|payment)/i.test(source)) {
    announcement = /all\s+major\s+cards/i.test(source)
      ? 'All major cards and Apple Pay now work at checkout.'
      : 'Card checkout and Apple Pay now work.';
  } else if (/(?:card\s+checkout|checkout\s+(?:is\s+)?(?:fixed|ready)|accept\s+all\s+major\s+cards)/i.test(source)) {
    announcement = 'Card checkout now works.';
  }
  const minimum = minimumSpend > 0
    ? ` on orders of ${minimumSpend} dollars or more`
    : '';
  const thanks = humanStyle.has('thanks') ? ' Thank you.' : '';
  const footer = requireOptOut ? ` ${RULES.optOut.exactSuffix}` : '';
  const text = `${greeting}${announcement ? ` ${announcement}` : ''} Get ${couponPercent}% off${minimum} with {{code}} at ${linkUrl}.${thanks}${footer}`;
  const verdict = validator(text, { brandName, approvedProductCodes, requireOptOut });
  return verdict.ok ? { text: verdict.text, septets: verdict.septets } : null;
}
...
async function draftCandidates(input = {}, dependencies = {}) {
  ...
  const allowedKeys = new Set(['workflowType', 'productName', 'brandName', 'cadence', 'brief',
    'candidateCount', 'linkUrl', 'approvedProductCodes', 'currentMessage', 'styleTraits', 'couponCode', 'couponPercent', 'minimumSpend']);
  ...
  const requireOptOut = requiresOptOutFooter(workflowType);
  ...
  const couponCode = input.couponCode == null ? null : String(input.couponCode).trim();
  if (couponCode && !/^[A-Z0-9-]{4,16}$/.test(couponCode)) {
    throw new CopyDraftError('The verified coupon code is invalid.', 'CAMPAIGN_AI_COPY_INPUT_REJECTED', 400);
  }
  const minimumSpend = input.minimumSpend == null ? null : Number(input.minimumSpend);
  if (minimumSpend !== null && (!couponCode || !Number.isFinite(minimumSpend)
    || minimumSpend < 0 || minimumSpend > 100000)) {
    throw new CopyDraftError('The verified coupon minimum is invalid.', 'CAMPAIGN_AI_COPY_INPUT_REJECTED', 400);
  }
  const cadenceNote = cadencePhrase(input.cadence);
  const couponPercent = input.couponPercent == null ? null : Number(input.couponPercent);
  if (couponPercent !== null && (!couponCode || !Number.isInteger(couponPercent)
    || couponPercent < 1 || couponPercent > 99)) {
    throw new CopyDraftError('The verified coupon percentage is invalid.', 'CAMPAIGN_AI_COPY_INPUT_REJECTED', 400);
  }
  ...
  const currentMessage = assertCurrentMessage(input.currentMessage, linkUrl);
  ...
  const messages = [
    { role: 'system', content: buildSystemPrompt({ requireOptOut }) },
    {
      role: 'user',
      content: buildUserPrompt({ workflowType, productName, brandName, cadenceNote, brief, candidateCount: requested, linkUrl, currentMessage, styleTraits: input.styleTraits, couponCode, couponPercent, minimumSpend })
    }
  ];
  ...
  // A model occasionally returns prose instead of JSON, or three versions
  // that all trip the same deterministic copy rule. The old path stopped
  // there and made the owner rewrite a perfectly clear brief. Retry a bounded
  // three times, feeding back RULE IDENTITIES only. Rejected message text is
  // never placed in another prompt or returned to the app.
  for (let attempt = 1; attempt <= 3 && candidates.length === 0; attempt += 1) {
    const failedChecks = [...new Set(rejected.flatMap(item => item.failedChecks || []))];
    const requestMessages = messages.map(item => ({ ...item }));
    if (attempt > 1) {
      const feedback = failedChecks.length
        ? `Previous versions were discarded by these checks: ${failedChecks.join(', ')}. Write entirely new versions that pass them.`
        : 'The previous response was not a usable JSON array of compliant messages. Return entirely new versions in the required JSON shape.';
    ...
    for (const text of parseCandidates(result?.content)) {
      ...
      seen.add(text);
      if (!requireOptOut && text.includes(RULES.optOut.exactSuffix)) {
        rejected.push({
          failedChecks: ['repeated_opt_out_footer'],
          reasons: ['The VIP suggestion repeated an opt-out footer that this workflow does not require.'],
          bannedTerms: []
        });
        continue;
      }
      ...
      // A syntactically valid message can still silently lose the offer while
      // being rewritten. Coupon facts came from WooCommerce above, so require
      // every one of them before a candidate can reach the phone.
      const minimumPattern = String(minimumSpend).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const missingRequiredFacts = [
        ...(/apple\s+pay/i.test(String(currentMessage || '')) && !/apple\s+pay/i.test(verdict.text)
          ? ['apple_pay_fact_preserved'] : []),
        ...(/all\s+major\s+cards/i.test(String(currentMessage || ''))
          && !/all\s+major\s+cards/i.test(verdict.text)
          ? ['major_cards_fact_preserved'] : []),
        ...(couponCode ? [
          ...(!verdict.text.includes('{{code}}') ? ['coupon_code_placeholder_required'] : []),
          ...(couponPercent !== null && !verdict.text.includes(`${couponPercent}%`)
            ? ['coupon_percentage_required'] : []),
          ...(minimumSpend > 0 && !new RegExp(
            `\\b${minimumPattern}(?:\\.00)?\\s+dollars?\\b`, 'i'
          ).test(verdict.text)
            ? ['coupon_minimum_required'] : [])
        ] : [])
      ];
      if (missingRequiredFacts.length) {
        rejected.push({
          failedChecks: missingRequiredFacts,
          reasons: missingRequiredFacts.map(check => ({
            apple_pay_fact_preserved: 'The suggested copy omitted Apple Pay from the original announcement.',
            major_cards_fact_preserved: 'The suggested copy omitted that all major cards are accepted.',
            coupon_code_placeholder_required: 'The suggested copy omitted the verified coupon placeholder.',
            coupon_percentage_required: 'The suggested copy omitted the verified discount percentage.',
            coupon_minimum_required: 'The suggested copy omitted the verified minimum order.'
          })[check]),
          bannedTerms: []
        });
        continue;
  ...
  if (candidates.length === 0 && couponCode) {
    const fallback = verifiedCouponFallback({
      brandName,
      couponPercent,
      minimumSpend,
      linkUrl,
      ...
      validator,
      approvedProductCodes,
      requireOptOut
    });
    if (fallback) candidates.push(fallback);
  }

  if (lastTransportError && candidates.length === 0) {
    throw new CopyDraftError(
  ...
  return {
    ...
    reviewRequirements: [
      ...
      ...(requireOptOut ? ['verify_opt_out_language'] : ['verify_stop_processing_remains_active']),
