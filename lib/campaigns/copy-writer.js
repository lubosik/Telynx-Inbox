...
const { renderBusinessContext, renderSelfCheck, renderTechniques } = require('./copy-craft');
...
function assertBrief(brief) {
  ...
  const cleaned = String(brief).replace(/[\r\n\t]+/g, ' ').replace(/\s+/g, ' ').trim();
  ...
  if (!BRIEF_PATTERN.test(cleaned)) {
    throw new CopyDraftError(
      ...
      + 'Plain writing punctuation is fine; links and brackets around variables are not.',
      'CAMPAIGN_AI_COPY_INPUT_REJECTED', 400
    );
  }
