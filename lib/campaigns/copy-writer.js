 ...
 *
 * 4. Nothing that fails validation is surfaced. Rejected drafts are counted
 *    and their broken rule ids are reported, but their text is not returned.
 *    A reviewer must not be able to copy a rejected draft out of a response
 *    body.
 */
...
async function draftCandidates(input = {}, dependencies = {}) {
  ...
  for (const text of parseCandidates(result?.content)) {
    ...
    if (!verdict.ok) {
      // The text is deliberately NOT carried out of this function. A reviewer
      // must not be able to lift a rejected draft out of an API response.
      rejected.push({
        failedChecks: verdict.failedChecks,
        reasons: verdict.failures.map(item => item.reason)
      });
      continue;
    }
