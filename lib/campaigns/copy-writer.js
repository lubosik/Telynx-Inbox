...
async function draftCandidates(input = {}, dependencies = {}) {
  ...
  for (let attempt = 1; attempt <= 3 && candidates.length === 0; attempt += 1) {
    ...
    for (const text of parseCandidates(result?.content)) {
      ...
      if (couponCode && (!verdict.text.includes('{{code}}')
        || (couponPercent !== null && !verdict.text.includes(`${couponPercent}%`))
        || (minimumSpend > 0 && !new RegExp(`\\b${minimumSpend}(?:\\.00)?\\s+dollars?\\b`, 'i').test(verdict.text)))) {
        rejected.push({
          failedChecks: ['coupon_terms_preserved'],
          reasons: ['The suggested copy did not preserve every verified coupon term.'],
          bannedTerms: []
        });
        continue;
      }
