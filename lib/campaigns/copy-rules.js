...
const RULES = {
  ...
  length: {
    ...
    //
    // 306, not 320. A concatenated SMS spends 7 bytes of every segment on a
    // user-data header telling the handset how to reassemble it, which costs
    // 7 septets, so two joined segments hold 153 each and not 160. Setting
    // this to 320 would pass a message the carrier then splits into three.
    //
    // The owner asked for a message the single-segment limit could not hold:
    // greeting, first name, the product they bought, the month, "here's a
    // code for 15% off your next order", the code, "order here", and a link.
    // Measured at 207 septets, 47 over one segment, and no phrasing kept all
    // of it — the tightest attempt still ran 15 over.
    //
    // The cost is real and small: roughly 376 extra segments across the
    // audience, a few dollars. The recipient sees no difference, because
    // every modern handset reassembles a concatenated message and displays it
    // as one. What is NOT free is length itself, so the ceiling stays a
    // ceiling: two segments, and the validator still refuses a third.
    maxSeptets: 306,
    // Kept for anything that wants to know where one segment ends, and for
    // reporting how many segments a message will actually cost.
  ...
  allCaps: {
    // VIP is a standard customer-tier initialism, not shouting. Keeping the
    // exception here (rather than pretending it is a product code) means the
    // validator, the AI prompt and the research document stay in agreement.
    alwaysAllowedTokens: ['STOP', 'VIP'],
  ...
  //
  // The list below began as internal SKUs, so a customer who bought BAC water
  // was told "15% off P-WA10". The owner's reaction was the correct one: that
  // does not make any sense to the person reading it.
  //
  // These are the shop's OWN product names, taken from the live catalogue,
  // minus two: tesamorelin and ipamorelin are banned compound names and may
  // not be written to a customer whatever the store calls them.
  //
  // Every entry is 16 characters or fewer, which is what the message budget
  // allows once a greeting, a name, a month, an offer and a link are in it.
  // The longer blends carry a short label instead; see DISPLAY_NAMES in
  // lib/campaigns/personalise.js.
  defaultApprovedProductCodes: [
    'Melanotan I', 'Melanotan II', 'BAC Water', 'Glutathione', 'Semax', 'Selank',
    'BPC-157 + TB-500', 'KLOW', 'CJC-1295 + IPA', 'GHK + BPC + TB',
  ...
  checks: [
    { id: 'length_within_one_segment', title: 'The whole message, including brand, link and opt-out, fits 306 GSM-7 septets, which is two concatenated segments.' },
    ...
    { id: 'no_all_caps_shouting', title: 'No ALL-CAPS word except STOP, VIP and verified product codes.' },
  ...
  promptRules: [
    ...
    'Do not quote a quantity in stock, a currency amount, or a deadline. You may state a percentage only when the brief asks for one.',
