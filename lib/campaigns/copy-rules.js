 ...
 *
 * A NOTE ON WHAT IS NOT HERE. `defaultApprovedProductCodes` is empty on
 * purpose. Vici's verified SKU list is product data, not a compliance
 * constant, and inventing one here would let an unverified product code
 * through the ALL-CAPS and obfuscation checks. Callers must pass the codes
 * they have verified; passing none means no code is exempt, which is the
