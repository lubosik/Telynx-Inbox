...
function conditionHolds(condition, facts, segmentMembership) {
  switch (condition.dimension) {
    ...
    case 'product_purchased': {
      // ── A PARENT KEY MEANS THE PRODUCT, NOT A VARIATION NOBODY BUYS ────
      //
      // Keys are `productID:variationID`, and `:0` is the parent. Customers
      // never hold a parent key: buying RT means holding 551:566, 551:567 or
      // 551:568, one per size. So a rule saying "bought RT", which the model
      // writes as 551:0 because that is how the catalogue names the product,
      // matched NOBODY.
      //
      // Measured on the live database: 551:0 matched 0 of 973, while the three
      // variations matched 44. A campaign built that way would have looked
      // like an audience that does not exist rather than a rule that cannot
      // hit, and the operator would have widened a perfectly good segment
      // trying to fix it.
      //
      // A parent therefore matches any variation of itself. A specific
      // variation still matches only itself, so "bought the 20mg" stays exact.
      const owned = facts.productKeys || [];
      const ownedExact = new Set(owned);
      const ownedProducts = new Set(owned.map(key => String(key).split(':')[0]));
      const hit = condition.value.some(key => {
        const [productID, variationID] = String(key).split(':');
        return variationID === '0' ? ownedProducts.has(productID) : ownedExact.has(key);
      });
