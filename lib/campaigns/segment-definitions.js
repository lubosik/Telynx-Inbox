...
/**
 * Evidence for the restock-only segment.
 *
 * It carries the SAME `statedReason` and the SAME `copyBasis` as the pairing,
 * on purpose. There is one permitted message here and both lists are pointed at
 * it, so two nearly-identical constants would be two things to keep in step and
 * one of them would eventually be edited alone.
 *
 * What is different is the timing block. The pairing marks it `selection_only`,
 * meaning it chose the audience and may not appear in copy. Here it is
 * `exclusion_only`: the only thing timing did was hand other people to other
 * lists, and for the person in front of you it is either unreadable or a long
 * way off. `timingRead` and `notInThisList` say which, so a reader is never
 * left wondering whether the better timed list missed this person or correctly
 * left them out.
 */
function restockOnlyEvidence(row) {
  const productName = row.restock.productName || row.timing?.productName || null;
  const restockObservedAt = new Date(row.observedTime).toISOString();
  const lastOrderAt = row.restock.lastPurchaseAt ?? null;
  const purchaseCount = Number.isFinite(Number(row.restock.purchaseCount))
    ? Number(row.restock.purchaseCount)
    : null;
  return {
    detector: 'back_in_stock',
    ...
    statedReason: 'product_back_in_stock',
    restockObservedAt,
    restockedProductID: row.productID,
    restockedVariationID: row.variationID || null,
    earlierReturnsSeen: row.earlierReturnsSeen,

    // ---- what we know about this person's own buying ----------------------
    // Both are about THE ITEM THAT CAME BACK, not the parent reorder series.
    // That is the thing the message names, so it is the thing the evidence
    // counts.
    lastOrderAt,
    purchaseCount,

    // ---- the timing, and the honest admission that there is none ----------
    timingUse: 'exclusion_only',
    timingRead: row.timingRead,
    // The raw engine state when there was one, so a developer reading a stored
    // row a year from now can reconstruct the decision exactly.
    timingState: row.timingState,
    // Named in full rather than implied, because "why is this person not in
    // the better list" is the first question anybody will ask of this row.
    notInThisList: 'back_in_stock_nearly_due',

    copyBasis: RESTOCK_REORDER_COPY_BASIS,
    summary: describeRestockOnlyMember({
      productName,
      restockObservedAt,
      lastOrderAt,
      purchaseCount,
...
const SEGMENT_DEFINITIONS = Object.freeze({
  ...
  back_in_stock_nearly_due: Object.freeze({
    ...
    description:
      ...
      'who are also close to the point where they normally buy again, on the same day. The ' +
      'stock coming back is the reason to write to them and it is the ' +
      ...
      'also in "Nearly due to reorder", where there is nothing to send yet. Everybody else who ' +
      'bought the same returned product is in "Back in stock, everyone else who bought it", and ' +
      'nobody is ever in both, so nobody hears the same news twice. It stays empty until a ' +
  ...
  /**
   * THE RESTOCK ON ITS OWN, WITH NO TIMING AT ALL. Read
   * lib/campaigns/restock-only.js before touching it, and read the disjointness
   * argument there before touching the copy.
   *
   * This and `back_in_stock_nearly_due` are two halves of one event and they
   * are DISJOINT, unlike the two "due to reorder" lists, which are nested. The
   * reason is in restock-only.js and it is short: a person in both would get
   * two messages about one restock, and the nearly due one is strictly better
   * timed. Each description names the other and says nobody is in both, so the
   * split is on the screen rather than only in this repository.
   *
   * The description also has to hand the next person the rule about what a
   * message from here may say. It is the same rule the pairing carries, and it
   * is easier to keep here than anywhere else: with no timing to work from,
   * there is genuinely nothing to say beyond the fact that the thing is
   * available again.
   */
  back_in_stock_other_buyers: Object.freeze({
    key: 'back_in_stock_other_buyers',
    detector: 'back_in_stock',
    name: 'Back in stock, everyone else who bought it',
    description:
      'Everybody else who bought the exact product that has just come back in stock. The stock ' +
      'coming back is the reason to write to them and it is the only thing the message may be ' +
      'about, because that is a fact about our shelves and it is true of every person here. ' +
      'Nobody in this list is near the point where they normally buy again: either we cannot ' +
      'read their buying at all, which is most people who have only bought once, or we can read ' +
      'it and their moment is a long way off. So there is nothing about their own buying to work ' +
      'from, and nothing about it may go in the message. Anyone who is close to buying again is ' +
      'in "Back in stock, and nearly due to reorder" instead, anyone who has reached their usual ' +
      'moment is in "Due to reorder, everyone due", and nobody is in this list and the nearly ' +
      'due one at the same time. It stays empty until a product has actually been recorded going ' +
      'out and coming back, and an empty list is a true answer rather than a fault.',
    compute(input, now) {
      const { rows } = restockOnlyRows(input, { now });
      return rows.map(row => ({
        contactPhone: row.phone,
        contactID: row.restock.contactID ?? row.timing?.contactID ?? null,
        contactName: row.restock.contactName ?? row.timing?.contactName ?? null,
        inclusionEvidence: restockOnlyEvidence(row),
        // Taken from the restock candidate, which is the only stage every
        // member of this list is guaranteed to have passed through: most of
        // them have no reorder candidate at all.
        commercialClearance: commercialClearanceOf(row.restock)
      }));
    }
  }),
