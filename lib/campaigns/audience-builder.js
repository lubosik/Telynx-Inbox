...
async function buildFromSegment({
  ...
  if (couponCode) {
    if (!copy.includes('{{code}}') || !Number.isInteger(Number(discountPercent))) {
      throw new AudienceBuildError('Use {{code}} in the message and set the verified coupon percentage.',
        'CAMPAIGN_COUPON_MESSAGE_MISMATCH', 409);
    }
    const { verifyExistingCoupon } = require('./existing-coupon');
    await verifyExistingCoupon({
      code: String(couponCode).toUpperCase(), percent: Number(discountPercent),
      audienceSize: renderable.length, message: copy
    });
  }
  ...
  const { data, error } = await client.rpc('create_sms_campaign_draft', {
    ...
    p_audience_definition: {
      ...
      ...(couponCode ? { coupon_code: String(couponCode).toUpperCase() } : {}),
