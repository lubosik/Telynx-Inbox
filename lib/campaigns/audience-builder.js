...
async function buildFromSegment({
  ...
  const { phones: candidates, segments } = await fromSegments({ client, segmentKeys: keys, workspaceID });
  ...
  const probe = await personaliseCampaign({
    ...
    phones: phones.length ? phones : ['+10000000000'],
    percentOff: discountPercent || 15, dryRun: true
  }).catch(() => null);

  const renderable = probe
    ? phones.filter(phone => probe.rendered.some(row => row.phone === phone))
    : phones;
  ...
  const summary = {
    ...
    candidates: candidates.length,
    suppressedAsDuplicate,
    cannotRender: phones.length - renderable.length,
    dedupeDays,
    priorCampaigns,
  ...
  const { data, error } = await client.rpc('create_sms_campaign_draft', {
    ...
    p_recipients: renderable.map(phone => ({
      ...
      contact_id: null,
      contact_name_snapshot: null,
      inclusion_reason: { source: 'segment', segments: keys }
    })),
    p_actor_user_id: actorID
  });
