...
// Manual broadcasts have a three-day attribution window in attribution-policy.
// A public/shared code can be redeemed later through an unrelated promotion;
// coupon possession alone must not keep claiming revenue indefinitely.
const COUPON_ATTRIBUTION_WINDOW_MS = 3 * 24 * 60 * 60 * 1000;
...
async function campaignCouponRedemptions({ client, campaignID, workspaceID = 'vici' }) {
  ...
  for (const order of orders) {
    ...
    for (const rawCode of order.coupon_codes || []) {
      ...
      const shared = holders.length > 1 || !code.startsWith('vin-');
      ...
      const sentAt = Date.parse(recipient.delivered_at || recipient.sent_at || '');
      const orderedAt = Date.parse(order.created_at || '');
      if (!Number.isFinite(sentAt) || !Number.isFinite(orderedAt)
        || orderedAt < sentAt || orderedAt - sentAt > COUPON_ATTRIBUTION_WINDOW_MS) {
        // Codes may be created at approval, before any SMS is sent. A buyer
        // who found or received the code elsewhere is not a campaign sale.
        continue;
      }
...
module.exports = { UNPAID_STATUSES, COUPON_ATTRIBUTION_WINDOW_MS, campaignCouponRedemptions };
