...
const WINBACK_NAMED = 'It\'s Vin from Vici. {{first_name}}, {{code}} is 15% off '
  + '{{last_product}}: {{product_link}} Reply STOP to opt out.';

// For the few whose order holds nothing that may be named. Not a degraded
// version of the message: it makes the same offer and names the same action,
// and only the product reference is missing.
// No product and no link: for the few whose order holds nothing that may be
// named, and the few whose product page cannot be linked. It makes the same
// offer and names the same action; only the reference is missing.
const WINBACK_PLAIN = 'It\'s Vin from Vici. {{first_name}}, {{code}} takes 15% off your next '
  + 'order. Your last was {{last_order_date}}. Order again when you like. Reply STOP to opt out.';
