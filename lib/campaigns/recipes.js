...
const CHECKIN_NAMED = 'Hi {{first_name}}, it\'s Vin from Vici. Just saw your {{last_product}} order went '
  + 'out a few weeks back. How are you finding it? Reply STOP to opt out.';

const CHECKIN_PLAIN = 'Hi {{first_name}}, it\'s Vin from Vici. Just saw your order went out a few '
  + 'weeks back. How are you finding everything? Reply STOP to opt out.';
...
const RECIPES = Object.freeze({
  ...
  checkin_21_day: Object.freeze({
    ...
    // This recipe has a bank of hand-written wordings in checkin-variants.js,
    // chosen per person from their profile so nobody gets the same message
    // twice running. `copy` above stays as the fallback for every case where a
    // profile is not available yet; see groupByVariant in audience-builder.js.
    variants: 'checkin'
  })
});
