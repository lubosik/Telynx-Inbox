...
function assertReviewableCopy(message) {
  ...
  async function create(input, actor) {
    ...
    if (couponCode) {
      if (!Number.isInteger(discountPercent) || discountPercent < 1 || discountPercent > 99 ||
          !proposedMessage.includes('{{code}}')) {
        throw new CampaignRequestError('For an existing coupon, enter its percentage and put {{code}} in the message.',
          'CAMPAIGN_COUPON_MESSAGE_MISMATCH', 409);
      }
      assertDiscountMatchesMessage(proposedMessage, discountPercent);
      await verifyExistingCoupon({ code: couponCode, percent: discountPercent,
    ...
    const audienceDefinition = allContacts ? {
      ...
      ...(couponCode ? { coupon_code: couponCode, discount_percent: discountPercent } : {})
