...
const { IN_CHUNK_SIZE, selectIn } = require('../fetch-all-rows');
...
function createSegmentService({
   ...
   * each with its own reason, answered in one read.
   *
   * WHY THIS EXISTS SEPARATELY FROM member()
   *   member() answers for one segment, which is the right answer to the wrong
   *   question once the segments overlap. Measured on the live workspace: 1,607
   *   memberships belong to only 517 people. 511 of them are in more than one
   *   segment and 205 are in four. Reading a person one segment at a time means
   *   opening four screens and never being told there were four.
   *
   *   Most of that overlap is correct rather than accidental. `one_time_lapsed`
   *   is a smaller circle drawn inside `one_time_buyers`, so the same person
   *   belongs in both and both reasons are true at once. The failure was never
   *   that the reasons were missing, it is that nothing put them side by side.
   *
   * ARCHIVED SEGMENTS ARE INCLUDED, AND MARKED
   *   Leaving them out would answer "which segments would message this person",
   *   which is a different question and one contactability already answers.
   *   Somebody asking why a person keeps appearing needs the archived group
   *   that still holds them, so it is returned with `archived: true` and the
   *   caller decides.
   *
   * CONTACTABILITY IS RESOLVED ONCE
   *   Whether this person may be messaged is a fact about the person, not about
   *   each segment they happen to sit in. Asking per segment would be four
   *   identical lookups and would invite a screen that implies the answer could
   *   differ between them.
   */
  async function personMemberships(rawPhone) {
    const phone = requiredPhone(rawPhone);

    const { data: memberRows, error: memberError } = await db()
      .from('sms_campaign_segment_members').select('*')
      .eq('workspace_id', workspaceID).eq('contact_phone', phone)
      .order('segment_id');
    if (memberError) throw databaseError(memberError, 'SEGMENT_MEMBER_LOAD_FAILED');

    const { data: overrideRows, error: overrideError } = await db()
      .from('sms_campaign_segment_overrides')
      .select('id,segment_id,contact_phone,override_type,reason,created_by,created_at,revoked_at,revoked_by,revoke_reason')
      .eq('workspace_id', workspaceID).eq('contact_phone', phone)
      .order('created_at', { ascending: false }).limit(200);
    if (overrideError) throw databaseError(overrideError, 'SEGMENT_OVERRIDE_LOAD_FAILED');

    // An override with no membership row still puts somebody in a segment, so
    // the segment ids come from both sides. Reading only memberRows would drop
    // exactly the people a human intervened on.
    const segmentIDs = [...new Set([
      ...(memberRows || []).map(row => row.segment_id),
      ...(overrideRows || []).filter(row => !row.revoked_at).map(row => row.segment_id)
    ])].filter(id => id !== null && id !== undefined);

    if (segmentIDs.length === 0) {
      return { contactPhone: phone, contactability: null, memberships: [], total: 0 };
    }

    // selectIn(), not a bare `.in()`: it serialises every value into the URL and
    // a long list overflows it. See lib/fetch-all-rows.js for the outage that
    // took the inbox down.
    //
    // It cannot carry the workspace filter, so that is applied here instead of
    // being dropped. The ids already came from rows scoped to this workspace,
    // so this is belt and braces rather than a correction.
    let segmentRows;
    try {
      segmentRows = await selectIn(
        db(), 'sms_campaign_segments', '*', 'id', segmentIDs
      );
    } catch (error) {
      throw databaseError(error, 'SEGMENT_LOAD_FAILED');
    }
    ...
    const overridesBySegment = new Map();
    for (const row of overrideRows || []) {
      if (!overridesBySegment.has(row.segment_id)) overridesBySegment.set(row.segment_id, []);
      overridesBySegment.get(row.segment_id).push(shapeOverride(row));
    }

    const membersBySegment = new Map((memberRows || []).map(row => [row.segment_id, row]));
    const contactability = await contactabilityFor([phone], new Date());

    const memberships = segmentIDs
      .map(segmentID => {
        const segmentRow = segmentsByID.get(segmentID);
        // A membership whose segment row is gone is not renderable and must not
        // become a half-empty card. Dropped, and the count below reflects it.
        if (!segmentRow) return null;
        const overrides = overridesBySegment.get(segmentID) || [];
        const memberRow = membersBySegment.get(segmentID) || null;
        return {
          segment: shapeSegment(segmentRow),
          archived: Boolean(segmentRow.archived_at),
          member: memberRow ? shapeMember(memberRow) : null,
          activeOverride: overrides.find(row => !row.revokedAt) || null,
          overrideHistory: overrides
        };
      })
      .filter(Boolean)
      // Live segments first, then by size: the biggest group a person is in is
      // usually the one that explains the rest.
      .sort((a, b) => Number(a.archived) - Number(b.archived)
        || (b.segment.memberCount || 0) - (a.segment.memberCount || 0)
        || String(a.segment.segmentKey || '').localeCompare(String(b.segment.segmentKey || '')));

    return {
      contactPhone: phone,
      // One answer, at the top, because it is about the person.
      contactability: contactability.byPhone.get(phone) || null,
      memberships,
