...
const { namedCouponInBrief, minimumInMessage, verifyExistingCoupon } = require('./existing-coupon');
...
const PAYMENT_OFFER_COPY =
  'Vin from Vici: Card checkout and Apple Pay work now. ' +
  '20% off orders of 100 dollars or more with {{code}} at https://vicipeptides.com. Reply STOP to opt out.';
...
function audiencePlanIssue(error) {
  const code = error?.code || 'SEGMENT_PLAN_UNAVAILABLE';
  if (code === 'SEGMENT_NEEDS_CLARIFICATION') {
    return {
      code, message: error.message,
      nextSteps: [
        'Add the missing audience detail to your description, such as a product, order window, or spend threshold.',
        'Tap Plan it again to preview the revised audience.'
      ]
    };
  }
  if (code === 'SEGMENT_UNSUPPORTED_AUDIENCE') {
    return {
      code,
      message: 'LUKO cannot define that audience reliably from the customer data it currently has.',
      nextSteps: [
        'Describe it using recorded orders, products, spend, timing, or an existing segment.',
        'If you meant everyone, say All Contacts. Otherwise create the audience manually under Growth > Segments.'
      ]
    };
  }
  if (code === 'SEGMENT_DRAFT_REJECTED' || code === 'SEGMENT_RULES_INVALID') {
    return {
      code,
      message: 'The generated audience rules did not pass validation, so no segment was saved.',
      nextSteps: [
        'Simplify the audience description to one or two precise conditions and tap Plan it again.',
        'If it still fails, build the rules manually under Growth > Segments.'
      ]
    };
  }
  if (code === 'SEGMENT_AI_BUILDER_UNAVAILABLE' || code === 'SEGMENT_AI_BUILDER_DISABLED') {
    return {
      code,
      message: 'The audience-writing service is unavailable right now; no segment was saved.',
      nextSteps: [
        'Tap Plan it again later, or build the segment manually under Growth > Segments.',
        'If the problem continues, ask an administrator to check the audience-writing service.'
      ]
    };
  }
  return {
    code: 'SEGMENT_PLAN_UNAVAILABLE',
    message: 'LUKO could not check that audience right now; no segment was saved.',
    nextSteps: [
      'Tap Plan it again once.',
      'If it still fails, use Growth > Segments or ask an administrator to check the campaign service.'
    ]
  };
}

function copyPlanIssue(error) {
  const code = error?.code || 'COPY_PLAN_UNAVAILABLE';
  if (code === 'ALL_DRAFTS_REJECTED') {
    return {
      code,
      message: 'The suggested messages did not pass the campaign copy checks; no draft campaign was created.',
      nextSteps: [
        'Rephrase the offer in plain, factual language and tap Plan it again.',
        'Or choose Write from scratch, start with Vin from Vici, and end with Reply STOP to opt out.'
      ]
    };
  }
  if (code === 'CAMPAIGN_AI_COPY_UNAVAILABLE' || code === 'CAMPAIGN_AI_COPY_DISABLED') {
    return {
      code,
      message: 'The copy-writing service is unavailable right now; no draft campaign was created.',
      nextSteps: [
        'Tap Plan it again later.',
        'Or choose Write from scratch and enter the message yourself.'
      ]
    };
  }
  return {
    code: 'COPY_PLAN_UNAVAILABLE',
    message: 'LUKO could not produce reviewable campaign copy from that description.',
    nextSteps: [
      'Simplify the message request and tap Plan it again.',
      'If it still fails, choose Write from scratch and enter your own message.'
    ]
  };
}
...
async function planCampaign({
  ...
  let discountPercent = explicitDiscount(text) ?? SHAPES[shape]?.discount ?? null;
  const couponCode = namedCouponInBrief(text);
  let couponError = null;
  let minimumSpend = null;
  if (couponCode) {
    try {
      const coupon = await verifyExistingCoupon({
        code: couponCode, percent: discountPercent,
        ...(couponLookup ? { lookup: couponLookup } : {})
      });
      if (discountPercent === null) discountPercent = Number(coupon.amount);
      minimumSpend = Number(coupon.minimum_amount || 0);
    } catch (error) {
      couponError = {
        code: error.code || 'CAMPAIGN_COUPON_NOT_READY',
        message: error.message,
        nextSteps: [`Check ${couponCode} in WooCommerce, then tap Plan it again.`]
      };
    }
  }
  const warnings = [];
  ...
  try {
    if (requestsAllContacts(text)) {
      ...
      if (!draft?.ruleSet || !preview) {
        const code = draft?.status === 'question' ? 'SEGMENT_NEEDS_CLARIFICATION'
          : draft?.status === 'unanswerable' ? 'SEGMENT_UNSUPPORTED_AUDIENCE'
            : 'SEGMENT_DRAFT_REJECTED';
        const detail = code === 'SEGMENT_NEEDS_CLARIFICATION'
          ? String(draft?.questions?.[0] || 'The audience description needs one more detail.').slice(0, 240)
          : 'The audience rules could not be drafted.';
        throw Object.assign(new Error(detail), { code, status: 422 });
      }
      audience = {
        kind: 'rules',
    ...
    audienceError = audiencePlanIssue(error);
  }
  ...
  let copyError = null;
  try {
    if (couponError) throw Object.assign(new Error(couponError.message), { code: 'COUPON_NOT_READY' });
    ...
    const result = await drafter({
      ...
      styleTraits,
      ...(couponCode ? { couponCode, minimumSpend, linkUrl: 'https://vicipeptides.com' } : {})
    });
    copy = (result.candidates || []).filter(candidate => !couponCode ||
      (candidate.text.includes('{{code}}') && candidate.text.includes(`${discountPercent}%`)
        && minimumInMessage(candidate.text, minimumSpend)));
    if (!copy.length) {
      copyError = copyPlanIssue({ code: 'ALL_DRAFTS_REJECTED' });
    }
  } catch (error) {
    copyError = copyPlanIssue(error);
  }

  if (!copy.length && requestsPaymentAnnouncement(text) && !couponError) {
    ...
    const fallback = couponCode && discountPercent === 20 && minimumSpend === 100 ? PAYMENT_OFFER_COPY
      : couponCode ? null : PAYMENT_ANNOUNCEMENT_COPY;
    const verdict = fallback ? validateCopy(fallback) : { ok: false };
    if (verdict.ok) {
  ...
  const nextSteps = [
    ...(audienceError?.nextSteps || []),
    ...(audience && audience.matchedCount === 0
      ? ['Broaden the product, order-date, or spend conditions and tap Plan it again.'] : []),
    ...(warnings.some(warning => warning.code === 'below_floor')
      ? [`Widen the audience to at least ${MINIMUM_MARKETING_AUDIENCE} matching contacts, or message the smaller group individually.`] : []),
    ...(copyError?.nextSteps || []),
    ...(couponError?.nextSteps || [])
  ];
  ...
  return {
    ...
    ready: Boolean(!couponError && audience && copy.length && audience.matchedCount > 0
      && !warnings.some(warning => warning.code === 'below_floor'))
  };
}
