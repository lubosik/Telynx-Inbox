...
const { whenToSendOffer } = require('./offer-timing');
...
const { mayOfferCheckInCode } = require('./checkin-offer-policy');
...
/** What the code is worth. Raised from 15 on the owner's call. */
const DISCOUNT_PERCENT = 20;

/**
 * The code itself, fixed and typeable.
 *
 * Not SMS20. That belongs to the win-back recipe, and reusing the string here
 * would make redemptions impossible to tell apart: coupon-attribution.js
 * credits revenue to whichever campaign owns a code, and issueSharedCode
 * re-restricts SMS20 to each campaign's audience as it runs. Two paths, one
 * code, and neither number means anything afterwards.
 *
 * A separate code costs nothing and keeps the answer to "did the check-in
 * follow-up actually earn anything" measurable.
 */
const OFFER_CODE = 'CHECKIN20';

/**
 * Total redemptions the shared code allows.
 *
 * couponSpec's `usageLimit` is the TOTAL across everybody and hardcodes
 * usage_limit_per_user to 1, so the default of 1 would let the first person to
 * redeem exhaust the code for everyone behind them — silently, with every
 * later customer seeing "coupon usage limit reached" at the checkout.
 *
 * Generous rather than unlimited: it still caps the damage if the code ever
 * escapes, and one person can only ever use it once.
 */
const OFFER_USAGE_LIMIT = 500;
...
function codeMessageTemplate(acknowledgement = FALLBACK_ACKNOWLEDGEMENT) {
  ...
  return `It's Vin from Vici. ${clause} {{code}} is 20% off your next order. `
...
async function handleCheckInReply({
  ...
  try {
    ...
    // ── WHO GETS AN OFFER ────────────────────────────────────────────────
    //
    // Not order count. mayIssueCode refused anybody with three or more paid
    // orders, which on the day this changed silently refused three of the four
    // genuinely happy replies, including the customer who wrote "I love the
    // Reta! Started it on the 10th and down 6lbs". Refusing your best
    // customers is a defensible policy and it is not the one that was asked
    // for, and it was happening invisibly.
    //
    // The question is whether they are LAPSED or simply between orders. See
    // checkin-offer-policy.js: somebody who redeemed a code has a recent order
    // and so gets a conversation rather than another discount, with no special
    // case written for it.
    const budget = await mayOfferCheckInCode({ client, phone, now });
    if (!budget.allowed) {
    ...
    // One code for everybody on this path, typeable over the phone. It is not
    // restricted by email: the person is replying to us, they have already had
    // a check-in, and a code somebody could guess is capped by its usage limit
    // rather than by who can read it.
    const code = OFFER_CODE;
    // Same two shapes the campaign path got wrong: a positional array rather
    // than { coupons }, and an expiry WooCommerce will accept, which is
    // ISO8601 with no timezone designator rather than a bare date.
    const expiresAt = `${new Date(now.getTime() + CODE_EXPIRY_DAYS * DAY_MS)
      .toISOString().slice(0, 10)}T23:59:59`;
    const spec = (coupons.couponSpec || couponSpec)({
      code, percent: DISCOUNT_PERCENT, usageLimit: OFFER_USAGE_LIMIT, expiresAt
    });

    // ── A SHARED CODE HAS TO BE REFRESHED, NOT JUST CREATED ──────────────
    //
    // The coupon is minted once and then reused by everybody who replies
    // after. Its expiry was fixed at that first mint, so somebody replying in
    // week four would be sent a code with two days left on it — an offer that
    // reads as generous and behaves as an insult.
    //
    // So an existing code has its expiry pushed out to this reply's own
    // window. Updated rather than recreated, because recreating would discard
    // the usage history and let somebody redeem twice.
    const existing = coupons.findCouponByCode
      ? await coupons.findCouponByCode(code).catch(() => null)
      : null;

    if (existing) {
      await (coupons.updateCoupon || updateCoupon)(existing.id, {
        amount: spec.amount,
      ...
      const minted = await coupons.createCoupons([spec]);
    ...
    //
    // This path sends the moment somebody answers, and the justification for
    // skipping the campaign gate is written at the top of this file: it is a
    // reply, seconds after the customer wrote first. That reasoning holds for a
    // thank-you and thins as the commercial content grows. At 20% off it is a
    // marketing message prompted by a reply, and one at 03:00 is still one at
    // 03:00.
    //
    // The REPLY is not held — somebody texting at 3am still gets answered then,
    // because that is a conversation. Only the offer waits for a civil hour.
    const timing = whenToSendOffer(now);
    if (!timing.sendNow) {
      await orderFlows().scheduleSMS({
        orderId: String(checkIn.campaign_id),
        phone,
        flowType: FLOW_TYPE,
        message: outcome.text,
        sendAt: timing.sendAt.toISOString()
      });
      return {
        sent: false,
        scheduled: true,
    ...
    // ── THE OFFER MUST APPEAR IN THE CONVERSATION ──────────────────────────
    //
    // sms_sent_log is a dedup ledger. sms_messages is what the inbox reads —
    // routes/conversations.js joins on contact_phone and filters nothing — so
    // writing only the ledger sent the customer a message that staff could not
    // see. A customer replied warmly, received the code, and sent a tapback
    // loving a message that was not in her thread. The shop owner: "I didn't
    // see the checkin20 text go out but they loved it."
    //
    // Only this path was affected. Every other send goes through sendAndLog in
    // flows/utils.js, or the delivery worker, both of which already write both
    // tables. The quiet-hours branch above is fine for the same reason: it
    // defers through scheduleSMS and comes back out through sendAndLog. That is
    // why the bug looked intermittent — the same offer was visible when it was
    // held until morning and invisible when it went straight out.
    //
    // Never throw from here. The SMS has already left, the dedup row is
    // already written, and this runs un-awaited inside the Telnyx webhook, so
    // a throw would lose the thread and gain nothing.
    ...
    try {
      await client.from('sms_messages').insert({
        ...
        status:            'sent',
        created_at:        sentAt,
        // The offer is part of the check-in campaign and is attributed to it,
        // the same way delivery-worker.js attributes a campaign send. Without
        // this the message shows in the thread but is invisible to campaign
        // analytics, so the campaign gets no credit for the reorder it buys.
        campaign_id:       checkIn.campaign_id || null
      });
      // So the thread sorts to the top of the inbox like any other reply.
      await client.from('sms_contacts').update({ last_seen: sentAt }).eq('phone', phone);
    } catch (inboxError) {
      console.error(
        `[CHECK-IN REPLY] offer sent but not shown in the inbox for ...${phone.slice(-4)}: `
        + inboxError.message
      );
    }
