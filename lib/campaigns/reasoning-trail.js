...
function cohortTrail(finding, { currency = 'USD' } = {}) {
  ...
  if (age && Number.isFinite(Number(age.middleDays))) {
    ...
    steps.push(`How long ago: typically ${Math.round(age.middleDays)} days, ranging from `
      + `${Math.round(age.earliestDays)} to ${Math.round(age.latestDays)}.`);
  }

  const stock = evidence.canStillBuyIt;
  if (stock && Number.isFinite(Number(stock.everyProductStillOnSale))) {
    steps.push(
  ...
  for (const blocker of finding.blockers || []) {
    steps.push(`In the way: ${blocker.detail || blocker.reason || blocker}`);
  }
