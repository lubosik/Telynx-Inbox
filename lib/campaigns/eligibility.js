...
function evaluateRecipient({
  ...
  if (contactDnd !== false || dndStatus !== 'inactive' || !Number.isFinite(nowTime)
...
async function loadCampaignSettings(client, workspaceID = WORKSPACE_ID) {
  const { data, error } = await client
    ...
    .select('workspace_id, drafts_enabled, provider_approved, live_send_enabled, consent_evidence_required, business_timezone, max_recipients_per_campaign, dnd_status_max_age_hours')
