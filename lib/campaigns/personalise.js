...
async function gatherFacts({ client, phones, skuMap: injectedSkuMap = null, linkMap: injectedLinkMap = null }) {
  ...
  for (const phone of unique) {
    ...
    if (!built.lastProductName && attempts.has(phone)) {
      ...
      built.attemptedProductLink = linkMap && attempt.sku
        ? (linkMap.get(String(attempt.sku).toUpperCase())
          || linkMap.get(`NAME:${String(attempt.label || '').toUpperCase()}`)
          || null)
        : (linkMap && attempt.label
          ? linkMap.get(`NAME:${String(attempt.label).toUpperCase()}`) || null
          : null);
    } else {
