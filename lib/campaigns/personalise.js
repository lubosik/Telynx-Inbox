...
// One definition of "this order was actually bought", imported rather than
// restated. sms_orders.status holds FULFILMENT states, and the live
// distribution is shipped 642, delivered 608, cancelled 252, failed 83,
// completed 47 — so a fifth private copy of that set is a fifth chance for one
// of them to drift.
const { PAID_STATUSES } = require('./segment-facts');
...
const DISPLAY_NAMES = Object.freeze({
  ...
  // ── THE SINGLE PRODUCTS, ADDED AFTER MEASURING WHY THEY FAILED ─────────
  //
  // Measured on the check-in batch of 8 September: messages naming a product
  // in its long form failed 3 of 3. Messages naming one by short code failed 1
  // of 60. Under the short-code rate, three-for-three is about a 1-in-200,000
  // coincidence, so this is the carrier reading "GHK-Cu" and "BPC-157" the way
  // it reads a compound name. Across all outbound history the same split holds
  // more mildly: 2.8% against 1.0% once payment messages are excluded.
  //
  // The blend above already rendered as "GHK + BPC + TB" and went out fine.
  // Only the single products were still long, purely because nobody had added
  // them here. This is the same defence as RENAMED_PRODUCTS and the banned
  // compound-name lexicon in copy-rules.js: the shop declining to spell out
  // what it sells is what keeps it deliverable.
  'GHK-CU': 'GHK',
  'BPC-157': 'BPC',
  'TB-500': 'TB',
...
function factsForBuyer({ contact, order, skuMap, linkMap = null, couponCode = null }) {
  ...
  for (const item of itemsByValue(order)) {
    ...
    if (label) {
      ...
      //
      // Same resolver as the attempted-product link, so a product that can be
      // NAMED is always also linkable. Three exact lookups used to leave a
      // drifted variation — KLOW against a catalogue holding KLOW80 — named
      // and unlinkable, which drops that person from the campaign entirely.
      lastProductLink = storeLinkFor({ sku, label, skuMap, linkMap })
...
/** How many rows one PostgREST response will return before it stops silently. */
const READ_PAGE_SIZE = 1000;

/**
 * Chunked AND paged `.in()` reads.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * TWO SILENT FAILURES, NOT ONE
 *
 *   An unbounded `.in()` list overflows the request URL. That was the failure
 *   this function was written for, and chunking fixes it.
 *
 *   A response caps at 1000 rows and says nothing. That one was never fixed
 *   here, so every chunk quietly returned at most 1000 rows: 200 phone numbers
 *   need only five paid orders each to cross it. Both failures are silent, and
 *   both have already caused an outage in this codebase.
 *
 *   It mattered less when the only survivors were the latest order and a
 *   count. It matters now: gatherFacts exports the full list as `orderHistory`,
 *   so the first caller to total somebody's spend or measure their reorder
 *   rhythm from a truncated array gets a confidently wrong number with no
 *   error anywhere.
 *
 *   lib/profiles/profile-builder.js already does this correctly. Two files
 *   answering one question differently is how this codebase keeps hurting
 *   itself, so this is now the same answer.
 */
  ...
  for (let index = 0; index < values.length; index += IN_CHUNK_SIZE) {
    ...
    for (let from = 0; ; from += READ_PAGE_SIZE) {
      const { data, error } = await client
        .from(table).select(columns).in(column, slice)
 ...
 *
 * ── WHY THE WHOLE ORDER HISTORY COMES BACK ─────────────────────────────────
 *
 *   Every order row for every phone is ALREADY read here. It always was. The
 *   function then kept the newest one, kept a count, and threw the rest away —
 *   so anything that wanted a second opinion about a customer (a reorder
 *   rhythm, a spend total, what they buy repeatedly) had to go and read the
 *   same rows again from somewhere else.
 *
 *   `orderHistory` costs one extra column in the select and no extra query.
 *   It is NOT a merge field and cannot become one: merge-fields.js renders
 *   exactly six named fields and refuses everything else, so this rides
 *   alongside the facts in the same way `email` already does.
 */
async function gatherFacts({ client, phones, skuMap: injectedSkuMap = null, linkMap: injectedLinkMap = null }) {
  ...
  const [contacts, orders, skuMap] = await Promise.all([
    ...
    // `total` is read for orderHistory below. It is not a merge field and no
    // rendered message has ever contained it; it is here so that a caller
    // asking "what has this person spent" does not have to re-read the table.
    readIn(client, 'sms_orders', 'contact_phone, items, created_at, status, total', 'contact_phone', unique),
  ...
  // Kept separately from `counts` on purpose. `counts` feeds the {{order_count}}
  // merge field and counts EVERY row including the 252 cancelled and 83 failed
  // ones; changing that would change what a live campaign renders, which this
  // is not allowed to do. `history` is paid-only, which is what a profile or a
  // cadence calculation needs. Two questions, two answers, both stated.
  const history = new Map();
  /** Newest UNPAID order per phone. Only used when there is no paid one. */
  const attempts = new Map();
  for (const order of orders) {
    const phone = order.contact_phone;
    counts.set(phone, (counts.get(phone) || 0) + 1);
    // ── THE LATEST PAID ORDER, NOT THE LATEST ROW ────────────────────────
    //
    // This used to take the newest order of ANY status, so {{last_product}}
    // and {{last_order_date}} could name an order the customer never
    // received. Measured on today's win-back: 12 of the 367 people it reached
    // had an unpaid newest order, and the copy that went to them read like
    //
    //     Saw you ordered RT in May
    //
    // where the May order was cancelled and their real last purchase was
    // February. Telling somebody you saw them buy a thing they cancelled is
    // worse than naming nothing, and this function's own docstring already
    // claimed it resolved the latest PAID order.
    //
    // Note `counts` above deliberately still counts every row: it feeds
    // {{order_count}}, which no live recipe uses, and changing it is a
    // separate decision from fixing a falsehood a customer can read.
    if (!PAID_STATUSES.has(String(order.status || '').toLowerCase())) {
      // The newest order that never completed, kept only so a campaign to
      // somebody with NO purchase can name what they tried to buy. It never
      // feeds {{last_product}}; see the attempted_product field.
      const heldAttempt = attempts.get(phone);
      if (!heldAttempt || new Date(order.created_at) > new Date(heldAttempt.created_at)) {
        attempts.set(phone, order);
      }
      continue;
    }
    ...
    const paid = history.get(phone);
    if (paid) paid.push(order); else history.set(phone, [order]);
  }
  // Newest first, matching `latest` above, so the head of orderHistory and the
  // order the merge fields were built from are the same row whenever the newest
  // order is a paid one.
  for (const paid of history.values()) {
    paid.sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at));
  }
  ...
  for (const phone of unique) {
    ...
    // Always an array, never undefined: a caller that has to test for the
    // absence of a list before iterating it eventually forgets to.
    built.orderHistory = history.get(phone) || [];

    // What they TRIED to buy, and only when they have bought nothing. For a
    // customer with a real purchase the interesting product is the one they
    // received, and populating both would create a second answer to "what
    // should this message name".
    //
    // Without this, all 122 people in the failed-order segment render an empty
    // {{last_product}} and are dropped as unpersonalisable — verified against
    // three of them before this existed, every one returning
    // missing: ["last_product"].
    if (!built.lastProductName && attempts.has(phone)) {
      const attempt = approvedLabelForOrder(attempts.get(phone), skuMap);
      built.attemptedProductName = attempt.label;
      built.attemptedProductSku = attempt.sku;
      built.attemptedOrderAt = attempts.get(phone).created_at;
      // The page for what they tried to buy. Named in the message and then
      // unreachable is the friction the message exists to remove.
      built.attemptedProductLink = storeLinkFor({
        sku: attempt.sku, label: attempt.label, skuMap, linkMap
      });
    } else {
      built.attemptedProductName = null;
      built.attemptedProductSku = null;
      built.attemptedOrderAt = null;
      built.attemptedProductLink = null;
    }
...
async function issueSharedCode({
  campaignID, phones, facts, percentOff, expiresInDays = DEFAULT_COUPON_DAYS, message = null,
  ...
  fixedCode = null,
  preserveFixedCode = false,
  // When true the coupon carries no email restriction, so anybody who types
  // the code gets the discount. See the note on the recipe: it trades a
  // visible, capped leak for an invisible checkout failure.
  publicCode = false,
  ...
  // Fail loudly rather than mint an unrestricted code BY ACCIDENT. A campaign
  // that meant to restrict and found no emails would otherwise create a public
  // discount silently, which is a different and much more expensive thing than
  // what was asked for. A campaign that CHOSE to be public is not that.
  if (!publicCode && !emails.length) {
    throw new PersonalisationError(
  ...
  if (preserveFixedCode) {
    // A named, already-live offer belongs to WooCommerce. Approval must never
    // silently change its expiry, total use limit or restrictions just because
    // an audience was frozen today. Recheck it immediately before approval.
    const { verifyExistingCoupon } = require('./existing-coupon');
    await verifyExistingCoupon({
      code, percent: pct, audienceSize: phones.length, message,
      lookup: coupons.findCouponByCode || findCouponByCode
    });
    return {
      code, byPhone: new Map(phones.map(phone => [phone, code])),
      withoutEmail: [], restrictedTo: null, publicCode: true, created: false
    };
  }
  const expiresAt = `${new Date(Date.now() + expiresInDays * DAY_MS)
    .toISOString().slice(0, 10)}T23:59:59`;
  ...
  const spec = (coupons.couponSpec || couponSpec)({
    ...
    // The ceiling is the audience either way: one redemption per person is
    // the intent, and it caps what a leaked public code can cost.
    usageLimit: Math.max(1, publicCode ? phones.length : emails.length),
    expiresAt,
    ...(publicCode ? {} : { emailRestrictions: emails })
  });
  ...
  if (existing) {
    await coupons.updateCoupon(existing.id, {
      ...
      // Explicitly emptied when the code is public, so a coupon that used to
      // be restricted does not keep a previous campaign's list.
  ...
  return {
    code, byPhone, withoutEmail,
    restrictedTo: publicCode ? null : emails.length,
    publicCode
  };
}
...
async function personaliseCampaign({
    ...
    if (dryRun) {
      ...
      if (sharedCode) {
        const issued = await issueSharedCode({
          campaignID, phones: audience, facts, percentOff, expiresInDays, fixedCode, publicCode, preserveFixedCode,
          message: template,
          ...(coupons ? { coupons } : {})
        });
  ...
  const outcome = renderForRecipients({ template, recipients, approvedMinimumSpend, requireOptOut });
  ...
  return {
    ...
    couponsIssued: sharedOutcome ? (sharedOutcome.created === false ? 0 : 1) : codes.size,
...
/** Where somebody goes when we cannot name their exact product. */
const SHOP_PAGE = 'https://vicipeptides.com/shop/';

/**
 * The store page for a product, resolved the same several ways its NAME is.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * WHY THE EXACT SKU IS NOT ENOUGH
 *
 *   The link lookup was a single exact match on the order's SKU, while the
 *   name lookup had three fallbacks — the catalogue, the rename map, and
 *   resolveBySkuBase for a drifted variation. So a product could be named and
 *   not linked, which drops that person entirely.
 *
 *   Measured: an order carrying `KLOW` resolved the label KLOW perfectly and
 *   found no link, because the catalogue holds `KLOW80` — the variation SKU
 *   with its size suffix. skuBase strips exactly that, and was already being
 *   used one function away.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * AND IF NOTHING MATCHES, THE SHOP
 *
 *   The owner's instruction, and it is right: a link somewhere real always
 *   beats no link. A message that names a product and then offers nowhere to
 *   buy it is the friction the message exists to remove, and being dropped
 *   from the campaign over it is worse still.
 *
 *   The shop page is honest — it does not claim to be the product page — and
 *   it is one tap from everything.
 */
function storeLinkFor({ sku, label, skuMap, linkMap }) {
  if (!linkMap) return SHOP_PAGE;
  const key = String(sku || '').toUpperCase();

  // 1. The SKU exactly as the order carries it.
  if (key && linkMap.has(key)) return linkMap.get(key);

  // 2. The product's approved name, for an order that predates a rename and
  //    has no usable SKU at all.
  const byName = label ? linkMap.get(`NAME:${String(label).toUpperCase()}`) : null;
  if (byName) return byName;

  // 3. A drifted variation: KLOW against a catalogue holding KLOW80. Only when
  //    the base is unambiguous, for the same reason resolveBySkuBase refuses to
  //    guess — the wrong product page is worse than the shop.
  const base = skuBase(sku);
  if (base) {
    const matches = new Set();
    for (const [known, link] of linkMap) {
      if (known.startsWith('NAME:')) continue;
      if (skuBase(known) === base) matches.add(link);
    }
    if (matches.size === 1) return [...matches][0];
  }

  // 4. The catalogue's own name for this SKU, which may differ from the
  //    approved label a customer reads.
  const catalogueName = skuMap && key ? skuMap.get(key) : null;
  if (catalogueName) {
    const byCatalogue = linkMap.get(`NAME:${String(catalogueName).toUpperCase()}`);
...
/**
 * The approved, customer-readable label for an order, or null.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * WHY THIS IS EXPORTED RATHER THAN REIMPLEMENTED
 *
 *   The profile builder needs the same answer this file already computes, and
 *   reimplemented it as `String(item.name).trim()` — the RAW WooCommerce name.
 *   Measured against the live catalogue: the campaign renderer produces an
 *   approved label for 832 of 844 buyers; the reimplementation produced one
 *   for 3. So 829 buyers were routed to a "we don't know your product" message
 *   while the campaign path knew it perfectly well.
 *
 *   Names in the store are "RT - 10mg" and "BPC-157 + TB-500 - 10mg + 10mg".
 *   approvedProductLabel is an exact match against a fixed code list, so a raw
 *   name never matches, and `name || sku` short-circuits so a perfectly good
 *   SKU is blocked behind the unusable name — the exact trap documented forty
 *   lines above this one.
 *
 *   One function answers it now. A second copy of this chain is how the
 *   answers drift apart again.
 */
function approvedLabelForOrder(order, skuMap = null) {
  for (const item of itemsByValue(order)) {
    // Same normalisation as factsForBuyer above: uppercase, and a missing SKU
    // is a lookup that misses rather than a crash.
    const sku = item?.sku || null;
    const key = String(sku || '').toUpperCase();
    const resolved = (skuMap ? skuMap.get(key) : null)
      || renamedProductFor(item)
      || (sku && skuMap ? resolveBySkuBase(sku, skuMap) : null)
      || String(item?.name || '').trim()
      || null;
    if (!resolved) continue;
    const label = approvedProductLabel(displayNameFor(resolved))
      || approvedProductLabel(resolved);
    if (label) return { label, sku: sku || null };
  }
  return { label: null, sku: null };
}

module.exports = {
  SHOP_PAGE,
  storeLinkFor,
  approvedLabelForOrder,
  skuToProductLink,
  DISPLAY_NAMES,
  displayNameFor,
  RENAMED_PRODUCTS,
