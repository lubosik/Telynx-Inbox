 ...
 */
/**
 * The longest store link that may reach a message.
 *
 * 58, the longest permalink on the live catalogue
 * (product/cjc-1295-without-dac-ipa). Measured across 23 published products:
 * 36 to 58 characters, median 42.
 *
 * ONE constant, because this was declared twice — `maxLength: 33` on the field
 * and a bare `> 33` inside approvedStoreLink. Raising the field alone changed
 * nothing: every readable permalink still rendered EMPTY, silently moving all
 * 112 recipients to "cannot be personalised". The same number in two places,
 * and the stricter copy winning, is the fault this codebase keeps repeating.
 */
const MAX_STORE_LINK_CHARS = 58;
...
const FIELDS = Object.freeze({
  ...
  last_product: Object.freeze({
    description: 'What the customer would call the last thing they bought.',
    // 16, the longest approved product name ("BPC-157 + TB-500"). Anything
    // longer carries a short label instead; see DISPLAY_NAMES in
    // lib/campaigns/personalise.js. It is never truncated into nonsense.
    maxLength: 16,
    fallback: '',
    render: (facts) => approvedProductLabel(facts?.lastProductName || facts?.lastProductSku)
  }),
  /**
   * What they TRIED to buy and did not complete.
   *
   * A separate field from `last_product`, not a fallback inside it. That one
   * means "what they bought", and a message naming a product somebody never
   * received is a lie they can catch — twelve people were told "saw you
   * ordered RT in May" about a cancelled May order, and that is precisely the
   * mistake it now refuses to make.
   *
   * 125 contacts have no paid order at all, holding $18,932 of attempts, 71 of
   * them for RT. Without this field every one of them renders an empty
   * `last_product` and is dropped as unpersonalisable, so a campaign to them
   * cannot name the one thing worth naming. Verified: all three sampled
   * recipients came back with `missing: ["last_product"]`.
   *
   * Same approved-label rule as `last_product`, so a product nobody can name
   * in words the customer would recognise is still not named.
   */
  attempted_product: Object.freeze({
    description: 'What the customer tried to buy on an order that never completed.',
    maxLength: 16,
    fallback: '',
    render: (facts) => approvedProductLabel(facts?.attemptedProductName || facts?.attemptedProductSku)
  }),
  code: Object.freeze({
    description: 'A discount code issued to this one person.',
    // real codes are always 14: vin- plus ten characters. Declaring 16 spent
    // two septets of a 160-septet budget on slack that cannot occur.
    maxLength: 14,
    fallback: '',
    render: (facts) => sanitiseCode(facts?.couponCode)
  }),
  product_link: Object.freeze({
    description: 'A link to the page for the last thing they bought. Nothing if it is unknown.',
    // 33: `https://vicipeptides.com/?p=` is 28, and WordPress post ids on
    // this store are four digits today but increment across every post, so
    // five is a matter of time. Sized for it now rather than discovering it
    // when a new product silently stops being linkable.
    //
    // Fixed length whatever the product: these are WordPress
    // shortlinks (`?p=<id>`) rather than readable permalinks, because a
    // permalink runs to 58 and its slug grows with the product name — so the
    // longest product names produced the longest URLs and nothing else fitted
    // beside them. Measured, not guessed.
    //
    // Anything longer renders EMPTY rather than being truncated. Every other
    // field degrades gracefully when clipped — half a product name is still a
    // product name — but half a URL is a broken link, which is worse than no
    // link at all. An empty value moves that person to the copy that uses no
    // link, which is the correct outcome.
    maxLength: MAX_STORE_LINK_CHARS,
    fallback: '',
    render: (facts) => approvedStoreLink(facts?.lastProductLink)
  }),
  /**
   * The page for the product they TRIED to buy.
   *
   * Paired with `attempted_product` for the same reason it exists: somebody
   * with no completed order has no `lastProductLink`, so a recovery campaign
   * could name the product and then give them nowhere to go — the friction the
   * whole message exists to remove.
   */
  attempted_product_link: Object.freeze({
    description: 'A link to the page for the product they tried to buy. Nothing if it is unknown.',
    maxLength: MAX_STORE_LINK_CHARS,
    fallback: '',
    render: (facts) => approvedStoreLink(facts?.attemptedProductLink)
  }),
 ...
 */
/**
 * A product page link, or nothing.
 *
 * Re-checked here rather than trusted from the catalogue. This is the last
 * thing between a stored URL and a customer's phone, and the rule that matters
 * is the DESTINATION: https, and a host on the approved list. A link that
 * fails renders empty, which removes that one recipient rather than sending
 * them somewhere nobody approved.
 */


function approvedStoreLink(raw) {
  const value = String(raw || '').trim();
  if (!value) return '';
  let url;
  try { url = new URL(value); } catch { return ''; }
  if (url.protocol !== `${RULES.links.requiredScheme}:`) return '';
  if (!RULES.links.approvedHosts.includes(url.hostname.toLowerCase())) return '';
  // Fragment always dropped. The query string is dropped too, EXCEPT the
  // product-id shortlink form `?p=<digits>`, which copy-validator allows for
  // the same reason: it is a public post id, identical for everyone who bought
  // that product, and it says nothing about the person receiving it.
  const query = /^\?p=\d+$/.test(url.search) ? url.search : '';
  const clean = `${url.origin}${url.pathname}${query}`;
  // Never truncated. See the note on maxLength above.
  return clean.length > MAX_STORE_LINK_CHARS ? '' : clean;
}
...
function sanitiseCode(raw) {
  const value = String(raw || '').trim();
  // Validated case-insensitively and returned AS WRITTEN, so a memorable code
  // can read like a coupon. WooCommerce lowercases codes on both storage and
  // lookup, so "SMS20" in a message redeems the coupon stored as "sms20" —
  // verified against the live store rather than assumed.
  return /^[A-Za-z0-9-]{4,16}$/.test(value) ? value : '';
}
