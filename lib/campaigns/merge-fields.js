...
const FIELDS = Object.freeze({
  ...
  product_link: Object.freeze({
    ...
    // The longest permalink on the live store is 58 characters. Measured, not
    // guessed: worst-case length validation depends on this being right, and
    // the template was sized around it.
    //
...
function approvedStoreLink(raw) {
  ...
  // Query string and fragment dropped: they cost characters the budget does
  // not have and can carry tracking nobody here decided to add.
  const clean = `${url.origin}${url.pathname}`;
  // Never truncated. See the note on maxLength above.
  return clean.length > 58 ? '' : clean;
}
