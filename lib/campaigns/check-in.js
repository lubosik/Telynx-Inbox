'use strict';
/**
 * lib/campaigns/check-in.js — the 21-day post-purchase check-in, as a daily
 * batch that goes through the ordinary campaign approval.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * WHY A DAILY BATCH AND NOT A PER-ORDER TIMER
 *
 ...
 *
 *   So the check-in is a campaign. Every day the sweep collects everybody
 *   whose 21-day mark fell in the last day, builds one draft, and walks it
 *   through the standing automation approval. The whole gate applies for
 *   free, and the approval record still says the system acted under the
 *   owner's standing authorisation.
 * ═══════════════════════════════════════════════════════════════════════════
 *
...
/** How wide one sweep is. A daily window keeps the check-in close to day 21. */
const BATCH_WINDOW_DAYS = 1;
 ...
 */
const TEMPLATE = 'Hi {{first_name}}, it\'s Vin from Vici. Just saw your {{last_product}} order went '
  + 'out a few weeks back. How are you finding it? Reply STOP to opt out.';

/** The same question for somebody whose product has no approved code. */
const TEMPLATE_NO_PRODUCT = 'Hi {{first_name}}, it\'s Vin from Vici. Just saw your order went out a '
  + 'few weeks back. How are you finding everything? Reply STOP to opt out.';
 ...
 */
/**
 * Products that are consumables rather than a purchase worth asking about.
 *
 * ── WHY BACTERIOSTATIC WATER IS NOT A CHECK-IN ────────────────────────────
 *
 *   The shop owner: "if they just ordered bac water let's not send those texts
 *   out if possible to do that".
 *
 *   He is right, and the reason is the message itself. "You picked up Bac Water
 *   a few weeks back. How did it go?" is asking somebody how they got on with
 *   sterile water. It is the reconstitution fluid other products are mixed
 *   with, nobody has an experience of it, and asking makes the shop sound like
 *   it is not reading its own orders.
 *
 *   Only an order that is ENTIRELY this counts. Bac water alongside a real
 *   product is the normal way to buy it — 121 of 400 orders sampled — and
 *   those people are checked in on as usual, about the product they actually
 *   bought. It is the 17 in 400 who ordered nothing else who are skipped.
 */
const ACCESSORY_ONLY = /bacteriostatic|bac\s*water|syringe|needle|alcohol\s*(wipe|pad|swab)/i;

/** Line-item names from an order, whether items arrive parsed or as JSON. */
function itemNamesOf(order) {
  ...
  return items
    .map(item => String(item?.name || item?.product_name || (typeof item === 'string' ? item : '')))
    .filter(Boolean);
}

/**
 * Whether this order is nothing but consumables.
 *
 * An order whose items cannot be read returns FALSE, not true: an unreadable
 * order is an unknown one, and skipping a real customer because a JSON column
 * would not parse is the worse of the two mistakes.
 */
function isAccessoryOnlyOrder(order) {
  const names = itemNamesOf(order);
  if (!names.length) return false;
  return names.every(name => ACCESSORY_ONLY.test(name));
}
...
function checkInDueAt(order, { days = CHECK_IN_DAYS } = {}) {
  ...
  // Nothing to ask somebody about sterile water. See ACCESSORY_ONLY.
  if (isAccessoryOnlyOrder(order)) return null;
 ...
 *
 * A one-day look-back gives each six-hour automation tick the same set until a
 * campaign is created. Campaign history and recipient dedupe make retries
 * idempotent.
 */
