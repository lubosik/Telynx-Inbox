...
/**
 * The people due in this sweep, excluding anyone a check-in already reached.
 *
 * The exclusion reads past check-in CAMPAIGNS rather than a flag on the order,
 * so re-running the sweep produces an empty batch instead of a second draft
 * aimed at the same forty people. A draft counts as having asked: it is about
 * to be approved, and two drafts for one audience is the failure this avoids.
 */
async function dueForCheckIn({
  ...
  const candidates = selectDue(orders, { now, days, windowDays });
  if (!candidates.length) return { due: [], alreadyAsked: 0, considered: 0 };
  ...
  if (campaignError) throw new Error(`Reading check-in campaigns failed: ${campaignError.message}`);

  const asked = new Set();
  for (const campaign of (campaigns || []).filter(row => row.status !== 'cancelled')) {
    for (let from = 0; ; from += 1000) {
      const { data } = await client
        ...
        .range(from, from + 999);
      for (const row of data || []) asked.add(row.contact_phone);
      if (!data || data.length < 1000) break;
    }
  }

  const due = candidates.filter(entry => !asked.has(entry.phone));
  return { due, alreadyAsked: candidates.length - due.length, considered: candidates.length };
}
