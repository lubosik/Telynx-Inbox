...
function checkLength(text) {
  ...
  return [failure(
    ...
    `This message is ${septets - RULES.length.maxSeptets} standard character${septets - RULES.length.maxSeptets === 1 ? '' : 's'} over the two-part campaign text limit. Shorten it by removing at least that many ordinary letters, numbers, spaces, or punctuation marks.`,
  ...
  for (const [index, character] of characters.entries()) {
    ...
    if (FORBIDDEN_CONTROL.includes(character)) {
      ...
      failures.push(failure(
        ...
        `Remove the hidden character at position ${index + 1}. Use an ordinary space instead of a line break, tab, or pasted control character.`,
    ...
    failures.push(failure(
      'gsm7_character_set_only',
      `Replace or remove ${JSON.stringify(character)} at position ${index + 1}. Campaign texts can use standard letters, numbers, and simple punctuation only.`,
      { character, position: index + 1 }
    ));
  }
  return failures;
...
/**
 * A greeting the brand may sit behind, and nothing else.
 *
 * "Hi Adrian, it's Vin from Vici, saw you ordered..." names the sender in four
 * words and reads like a person. The character limit above refused it, because
 * a first name is a variable and the brand's position moves with it.
 *
 * Raising the limit to 20 or 30 was the wrong fix, and this file already said
 * so: at that distance the brand can sit behind a whole clause and "identified
 * up front" stops meaning anything. So the exception is not a bigger number,
 * it is a SHAPE — a greeting word, a name, a comma. Nothing else may precede
 * the brand, which is the requirement the character count was standing in for.
 *
 * The name is capped at 20 characters and may not contain sentence
 * punctuation, so no amount of text can be smuggled in as a greeting.
 */
const PERMITTED_GREETING = /^(?:hi|hello|hey)\b[^.!?]{0,20}?,\s*/i;
...
function checkBrandPrefix(text, brandName) {
  ...
  // After a greeting, the same lead-in applies to what follows it.
  const greeting = String(text).match(PERMITTED_GREETING);
  if (greeting) {
    const afterGreeting = String(text).slice(greeting[0].length);
    const indexAfter = afterGreeting.indexOf(brandName);
    if (indexAfter >= 0 && indexAfter <= BRAND_LEAD_IN_LIMIT) return [];
  }
  return [failure(
    'brand_identifies_sender_first',
    `Message must name the brand "${brandName}" within the first ${BRAND_LEAD_IN_LIMIT} characters, `
    + 'or immediately after a greeting like "Hi Adrian, ", so the sender is identified before anything else.'
  )];
}
...
function approvedMinimumPhrase(text, amount) {
  ...
  const expression = new RegExp(
    `\\borders?(?:\\s+of)?\\s+\\$\\s*${value.replace('.', '\\.')}\\s+(?:or\\s+more|minimum)\\b`,
    'i'
  );
  return String(text || '').match(expression)?.[0] || null;
}

function checkPatterns(text, { approvedMinimumSpend = null } = {}) {
  const failures = [];
  for (const entry of BANNED_PATTERNS) {
    // A verified coupon's checkout minimum is not an invented price claim.
    // Allow only the narrow, plain-language shape the customer needs in order
    // to use the offer. The caller must supply the WooCommerce-backed amount;
    // without it the ordinary currency ban remains unchanged.
    if (entry.id === 'price_or_percentage_offer') {
      const allowed = approvedMinimumPhrase(text, approvedMinimumSpend);
      if (allowed) {
        const withoutAllowedMinimum = String(text).replace(allowed, 'orders');
        entry.regex.lastIndex = 0;
        const otherCurrency = entry.regex.exec(withoutAllowedMinimum);
 ...
 * @param {number} [options.approvedMinimumSpend]   WooCommerce-verified order
 *   minimum allowed only in the phrase "orders $X or more".
 * @param {boolean} [options.requireOptOut]         Defaults true. A false
 *   value is reserved for workflows covered by opt-out-policy.js; it never
 *   changes provider STOP processing or recipient suppression.
...
function validateCopy(text, options = {}) {
  ...
  const requireOptOut = options.requireOptOut !== false;
  ...
  const failures = dedupe([
    ...
    ...(requireOptOut ? checkOptOutSuffix(text) : []),
