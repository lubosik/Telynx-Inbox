...
function privateCallLog(log) {
  ...
  // Available means "the app can play this", not "it is already in our bucket".
  //
  // Recordings archive on demand: GET /api/voice/recordings/:id pulls from the
  // provider into private storage the first time anyone presses play. Gating on
  // storagePath alone reported 43 of 44 real recordings as missing, because
  // nothing had triggered an archive yet — the call had a recording, the
  // endpoint could serve it, and the UI showed nothing.
  //
  // A recording_id is the provider's handle, so it is the honest signal that
  // audio exists. A deleted recording is gone for good and stays unavailable.
  const available = Boolean((storagePath || _recordingID) && !log.recording_deleted_at);
...
async function purgeExpiredRecordings({
  ...
  for (const row of rows || []) {
    try {
      ...
      // The destruction of evidence has to outlive the evidence. Until now a
      // successful purge left no trace at all — only failures were logged, and
      // only to the console. The audit row lives in a table with no delete
      // path, so the recording is gone at 30 days while the record that we
      // destroyed it survives for years.
      await logAudit({
        eventType: 'recording.purged',
        actorType: 'system',
        entityId: row.id,
        summary: `Purged the expired call recording for call log ${row.id}`,
        previousState: { recording_archived: Boolean(row.recording_storage_path), recording_deleted_at: null },
        newState: { recording_archived: false, recording_deleted_at: now.toISOString() },
        changedFields: ['recording_storage_path', 'recording_url_mp3', 'recording_url_wav', 'recording_deleted_at'],
        metadata: {
          call_log_id: row.id,
          expired_at: row.recording_expires_at,
          storage_removed: Boolean(row.recording_storage_path),
          provider_deleted: Boolean(row.recording_id),
