 ...
 * @param {boolean} [opts.throwOnCeiling]  what to do AT that ceiling.
 *
 *   Default `false` keeps the historic behaviour: warn and return the rows read
 *   so far. That is right for a screen, where showing the newest 100,000
 *   conversations beats showing an error.
 *
 *   It is wrong for anything that reasons about absence. A caller that reads a
 *   table to find out who has withdrawn consent, and silently receives a
 *   truncated list, concludes "no withdrawal on file" for everybody past the
 *   ceiling. `scripts/backfill-order-sms-consent.js` is exactly that caller and
 *   passes `throwOnCeiling: true`, so a truncated read aborts the run instead
 *   of writing consent records for people who said no.
 *
...
async function fetchAllRows(supabase, table, columns, opts = {}) {
  ...
  // Reaching here means page `maxRows / PAGE_SIZE` came back full: there is more
  // data and it was not read.
  if (throwOnCeiling) {
    throw new Error(
      `${table} exceeded the ${maxRows}-row ceiling; refusing to return a partial view.`);
  }
