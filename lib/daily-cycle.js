...
async function draftProposalsFor({ opportunities, proposals, drafter, env, dryRun, log }) {
  ...
  let drafted = 0;
  let saved = 0;
  // Genuinely new, as opposed to refreshed. This is the number the digest is
  // allowed to treat as news; see the note in proposal-service.saveBatch.
  let created = 0;
  const failures = [];
  ...
  return { drafted, saved, created, wouldDraft: opportunities.length, disabled: false, dryRun: false, failures };
}
  ...
  for (const user of users) {
    ...
    try {
      digest = prepareDailyDigest({
        ...
        // `created`, not `saved`. `saved` counts the upsert, which refreshes
        // every pending proposal every day, and the digest bypasses its own
        // novelty gate whenever this is above zero — so using `saved` would
        // push the same "6 proposals are waiting for review" every morning.
        // A proposal is news the day it appears and not after.
        proposalsDrafted: Number(cycleSummary.proposals?.created || 0),
