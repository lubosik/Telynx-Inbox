...
const ROUTE_POLICY = [
  ...
  { method: 'GET',    path: '/api/cart-recovery',                permission: 'automation.read' },
  { method: 'GET',    path: '/api/cart-recovery/journeys',       permission: 'automation.read' },
  { method: 'GET',    path: '/api/cart-recovery/journeys/:id',   permission: 'automation.read' },
  { method: 'GET',    path: '/api/cart-recovery/settings',       permission: 'automation.read' },
  { method: 'GET',    path: '/api/cart-recovery/voices',         permission: 'automation.read' },
  { method: 'POST',   path: '/api/cart-recovery/voices/:voiceId/preview', permission: 'automation.read', audit: true },
  { method: 'POST',   path: '/api/cart-recovery/journeys/:id/voice-attempt/preview', permission: 'automation.read', audit: true },
  { method: 'PUT',    path: '/api/cart-recovery/settings',       permission: 'campaigns.approve', audit: true },
  { method: 'PATCH',  path: '/api/cart-recovery/journeys/:id/replies/:replyId/draft', permission: 'message.send', audit: true },
  { method: 'POST',   path: '/api/cart-recovery/journeys/:id/replies/:replyId/approve', permission: 'message.send', audit: true },
  { method: 'POST',   path: '/api/cart-recovery/journeys/:id/replies/:replyId/discard', permission: 'message.send', audit: true },
  ...
  { method: 'GET',    path: '/api/analytics/cart-recovery',      permission: 'analytics.read' },
  { method: 'GET',    path: '/api/analytics/vip-leaderboard',    permission: 'analytics.read' },
  ...
  // The do not contact list. Reading it costs the same as reading a segment;
  // changing it costs the same as changing one, because deciding who a business
  // will never message again is a campaign decision.
  { method: 'GET',    path: '/api/do-not-contact',                permission: 'campaigns.read' },
  { method: 'POST',   path: '/api/do-not-contact',                permission: 'campaigns.manage', audit: true },
  { method: 'DELETE', path: '/api/do-not-contact/:phone',         permission: 'campaigns.manage', audit: true },
  ...
  // The standing authorisation for the automatic 21-day check-in.
  //
  // `campaigns.approve`, not `campaigns.manage`. Switching this on is not
  // managing a campaign: it authorises the system to approve and send future
  // check-ins with no human reading them. That is an approval act, and it
  // should need the same permission as approving one campaign by hand.
  { method: 'GET',    path: '/api/campaigns/automations/check-in', permission: 'campaigns.read' },
  { method: 'PUT',    path: '/api/campaigns/automations/check-in', permission: 'campaigns.approve', audit: true },
  ...
  { method: 'POST',   path: '/api/campaigns/coupons',           permission: 'campaigns.approve', audit: true },
  ...
  // Editing the audience is a pre-approval act, so it needs the same
  // permission as managing the campaign rather than approving one.
  { method: 'POST',   path: '/api/campaigns/:id/recipients/:recipientId/deselect', permission: 'campaigns.manage', audit: true },
  { method: 'POST',   path: '/api/campaigns/:id/recipients/deselect-excluded', permission: 'campaigns.manage', audit: true },
  ...
  { method: 'POST',   path: '/api/campaigns/:id/reschedule',     permission: 'campaigns.launch', audit: true },
  { method: 'POST',   path: '/api/campaigns/:id/cancel',         permission: 'campaigns.cancel', audit: true },
  { method: 'POST',   path: '/api/campaigns/check-copy',         permission: 'campaigns.manage' },
  { method: 'POST',   path: '/api/campaigns/:id/dry-run',        permission: 'campaigns.manage' },
  // Reads only, and mints nothing, so `campaigns.read` rather than manage: a
  // reviewer who may look at a campaign must be able to see what it actually
  // says to people, which is the whole point of reviewing it.
  { method: 'POST',   path: '/api/campaigns/:id/preview',        permission: 'campaigns.read' },
  // The recipe catalogue is static description, readable by anyone who may see
  // campaigns at all.
  { method: 'GET',    path: '/api/campaigns/recipes',            permission: 'campaigns.read' },
  // Building creates draft campaigns, so `campaigns.manage`, the same
  // permission as creating one by hand. It cannot approve, schedule, mint or
  // send, so it is deliberately NOT gated on campaigns.approve or launch.
  { method: 'POST',   path: '/api/campaigns/build',              permission: 'campaigns.manage', audit: true },
  // Planning writes nothing at all, so it needs only the permission to read
  // campaigns; accepting a plan creates a segment and a draft, so it needs the
  // same permission as creating either by hand.
  { method: 'POST',   path: '/api/campaigns/plan',               permission: 'campaigns.read' },
  { method: 'POST',   path: '/api/campaigns/plan/accept',        permission: 'campaigns.manage', audit: true },
  ...
  // The app has always called these two. They were never declared, so the
  // enforcer default-denied them and archiving was impossible from the phone.
  // A real send to a real handset. campaigns.approve, not manage: anybody who
  // can text an arbitrary number can text a customer.
  { method: 'POST',   path: '/api/campaigns/:id/test-send',        permission: 'campaigns.approve', audit: true },
  { method: 'POST',   path: '/api/campaigns/:id/archive',          permission: 'campaigns.manage', audit: true },
  { method: 'POST',   path: '/api/campaigns/:id/unarchive',        permission: 'campaigns.manage', audit: true },
