 ...
 *      permission". It is spelled explicitly and used for exactly two
 *      endpoints, so a MISSING entry can never be mistaken for an intentional
 *      one. A request that matches no entry is denied — see
 *      lib/enforce-policy.js.
...
const ROUTE_POLICY = [
  ...
  // ── Team management ───────────────────────────────────────────────────────
  // These two are the ONLY `permission: null` entries in the table. Every
  // authenticated actor, including a Support Agent, may read their own profile
  // and change their own password — that is also how a must_change_password
  // account escapes the forced-rotation lock.
