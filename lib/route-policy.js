 ...
 *
 *      THE SAME APPLIES TO THE CUSTOMER-FACING OPT-IN, AND MORE SO:
 *
 *        GET  /sms-optin                       serves a static page; the server
 *                                              never reads the token
 *        POST /sms-optin/confirm               holds an opt-in invitation token
 *        POST /sms-optin/decline               holds an opt-in invitation token
 *
 *      These belong to a CUSTOMER rather than to staff. There is no account
 *      behind them and never will be, so a session is not merely absent, it is
 *      impossible. They are mounted on '/sms-optin' in server.js, outside the
 *      '/api' prefix the enforcer guards, and they are rate limited in
 *      routes/sms-optin.js. Adding them here would fail
 *      test/route-policy.test.js as a dangling rule and would tighten nothing.
 ...
 *
 *      DELIBERATELY UNFLAGGED, AND WHY: `PATCH /api/users/me`,
 *      `POST /api/users/me/email` and `POST /api/users/me/email/cancel` have
 *      fully written audit call sites, but the two event types they need —
 *      `team.member.profile_updated` and `team.member.email_changed` — do not
 *      exist in lib/audit/event-types.js, and `logAudit` THROWS on an
 *      unregistered type. Those handlers route through `auditIfRegistered`,
 *      which consults the catalogue and warns instead of throwing, so today
 *      they write no row. Flagging them would assert coverage the code cannot
 *      provide, which is precisely the fault the enforcement above exists to
 *      stop. Register the two types, then set the flag here in the same change.
 *
