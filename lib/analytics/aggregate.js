...
function aggregateRevenue(attributions) {
  ...
  const recovered = deduplicate(attributions, row => String(row.order_id || row.id || ''))
    .filter(row => !row.invalidated_at && ['payment_recovery', 'cart_recovery'].includes(row.category))
...
const REVENUE_DRIVER_META = Object.freeze({
  cart_recovery: Object.freeze({ key: 'cartRecovery', label: 'Abandoned Cart Recovery' }),
...
function revenueDriverFor(row) {
  ...
  if (category === 'cart_recovery') return REVENUE_DRIVER_META.cart_recovery;
...
function aggregateRevenueDrivers(attributions) {
  ...
  const displayOrder = ['cartRecovery', 'paymentRecovery', 'campaigns', 'reorders', 'backInStock', 'winbacks'];
...
async function loadAnalyticsSource(client, range) {
  ...
  const queries = await Promise.all([
    ...
    fetchPaged(client, {
      ...
      filters: [query => query.eq('workspace_id', WORKSPACE_ID).in('category', ['payment_recovery', 'cart_recovery'])]
    }),
    fetchPaged(client, {
      table: 'sms_sent_log',
