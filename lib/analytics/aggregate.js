...
async function loadAnalyticsSource(client, range) {
  ...
  const queries = await Promise.all([
    fetchPaged(client, {
      ...
      columns: 'id,order_id,customer_id,currency,gross_amount,refunded_amount,net_amount,category,workflow,originating_action_type,originating_action_id,action_at,conversion_at,attribution_window_seconds,confidence_level,confidence_score,reason,supporting_evidence,is_refunded,invalidated_at,updated_at',
...
function createAnalyticsService({ client, now = () => new Date(), reliableFrom = process.env.ANALYTICS_RELIABLE_FROM || '2026-01-17' }) {
  ...
  return {
    async overview(params = {}) {
      ...
      const current = aggregatePeriod(source, range, rules.business_timezone);
      const previous = range.previous ? aggregatePeriod(source, range.previous, rules.business_timezone) : null;
      current.sentiment.changeFromPrevious = current.sentiment.averageScore === null || previous?.sentiment.averageScore === null
        ? null
        : Number((current.sentiment.averageScore - previous.sentiment.averageScore).toFixed(2));
      const result = {
        ...
        availability: availabilityFor(source, current, backfillComplete, coverage),
        warnings
      };
      return result;
    },
