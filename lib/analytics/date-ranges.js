...
function rangeForPeriod({
  ...
  if (period === 'custom') {
    ...
    startParts = { year: today.year, month: Math.floor((today.month - 1) / 3) * 3 + 1, day: 1 };
    end = now;
  } else if (period === 'year') {
  ...
  if (period !== 'all' && period !== 'custom') {
    ...
    if (period === 'today') {
      ...
      previousStartParts = shiftCivilMonths(startParts, -3);
      previousEndDate = { ...shiftCivilMonths(todayCivil, -3), ...wallTime };
    } else {
