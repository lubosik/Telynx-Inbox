...
async function listAccountVoices({ env = process.env, fetchImpl = global.fetch, timeoutMs = 15_000 } = {}) {
  ...
  try {
    ...
    return (data.voices || []).filter(voice => voice?.voice_id && voice?.name).map(voice => ({
      ...
      previewUrl: voice.preview_url || null,
      // A saved, shared Professional Voice Clone can be used by this account
      // even though its local verification flag is false: the owner verified
      // it before publishing, and this account has copied it from the library.
      // Never extend that rule to an unverified private clone or generated voice.
      verified: voice.category === 'premade'
        || voice.voice_verification?.is_verified === true
        || (voice.category === 'professional' && voice.sharing?.status === 'copied'
