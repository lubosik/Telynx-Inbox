...
// Account-scoped voices only. Automated customer calls must never offer an
// arbitrary community voice whose owner/licence has not been reviewed. The
// ElevenLabs account is the authorization boundary for Vin recovery voices.
async function listAccountVoices({ env = process.env, fetchImpl = global.fetch, timeoutMs = 15_000 } = {}) {
  const key = keyFrom(env);
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetchImpl(`${API}/voices`, {
      headers: { 'xi-api-key': key }, signal: controller.signal
    });
    if (!response.ok) throw Object.assign(new Error(`Voice catalogue failed (${response.status})`), {
      code: response.status === 401 ? 'VOICE_KEY_REJECTED' : 'VOICE_CATALOGUE_FAILED'
    });
    const data = await response.json();
    return (data.voices || []).filter(voice => voice?.voice_id && voice?.name).map(voice => {
      const ownerVerified = voice.category === 'professional'
        && voice.voice_verification?.is_verified === true;
      const copiedVerified = voice.category === 'professional'
        && voice.sharing?.status === 'copied'
        && voice.sharing?.original_voice_id === voice.voice_id
        && Boolean(voice.sharing?.public_owner_id)
        && voice.voice_verification?.requires_verification === false;
      const professionalClone = ownerVerified || copiedVerified;
      return {
        id: voice.voice_id,
        name: voice.name,
        provider: 'elevenlabs',
        providerLabel: 'ElevenLabs',
        modelId: env.ELEVENLABS_MODEL || DEFAULT_MODEL,
        ...
        descriptive: voice.labels?.description || voice.description || null,
        category: voice.category || null,
        sharingStatus: voice.sharing?.status || null,
        professionalClone,
        syntheticDesign: false,
        previewUrl: voice.preview_url || null,
        // Recovery calls accept only high-fidelity Professional Voice Clones.
        // A copied library voice is already verified by its owner even though
        // ElevenLabs reports is_verified=false in the receiving account.
        verified: professionalClone
      };
    });
  } finally {
    clearTimeout(timer);
...
module.exports = { DEFAULT_MODEL, DEFAULT_VOICE_ID, MAX_CHARACTERS, listAccountVoices, searchVoices, speak };
