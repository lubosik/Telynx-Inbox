...
function publicThread(row, { includeSummary = false } = {}) {
  if (!row) return null;
  return {
    id: row.id,
    ...
    summarisedMessageCount: Number(row.summarised_message_count || 0),
    // The summary TEXT, on the DETAIL view only.
    //
    // When a long conversation is compacted the older half stops being sent as
    // turns and is carried as this paragraph instead. Hiding it entirely means
    // somebody is told nothing about what happened to half their conversation
    // and cannot check that what was kept is right. It is their own thread and
    // the model wrote it about them, so they can read it.
    //
    // Withheld from the list, where forty rows would each carry a paragraph
    // nobody is reading, for a screen that shows a title and one line.
    ...(includeSummary ? { summary: row.summary || null } : {})
  };
}
...
async function getThread({ client, userId, threadId }) {
  ...
  return { thread: publicThread(row, { includeSummary: true }), messages };
}
