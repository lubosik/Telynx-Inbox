 ...
 *
 * The two rows go in one insert with explicit timestamps a millisecond apart,
 * rather than two inserts leaning on the default. Postgres `now()` is
 * transaction start time, so a single statement would stamp both rows
 * identically and leave their order to the uuid tiebreak, which is random. An
 * assistant turn sorting above the question it answers would corrupt every
 * later read of the thread, including the compaction boundary.
 */
    ...
    client.from(MESSAGES_TABLE).insert([
      {
        thread_id: row.id,
        role: 'user',
        content: String(question || '').slice(0, MAX_CONTENT_LENGTH),
        tools_used: [],
      ...
      },
      {
        thread_id: row.id,
        role: 'assistant',
        content: String(reply || '').slice(0, MAX_CONTENT_LENGTH),
