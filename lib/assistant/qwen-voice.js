...
function safeAudioURL(value) {
  ...
  // Model Studio currently returns signed DashScope OSS links with an http
  // scheme even from a workspace-scoped HTTPS request. OSS supports TLS, so
  // upgrade the provider link before downloading instead of accepting clear
  // text. Keep the host pattern narrow rather than trusting every Alibaba
  // subdomain.
  if (!/^dashscope-result-[a-z0-9-]+\.oss-[a-z0-9-]+\.aliyuncs\.com$/.test(host)
...
function isViciVoiceID(id) {
  return id.startsWith(VICI_PREFIX)
    || /^qwen-tts-vd-vici_[A-Za-z0-9_]{1,11}-voice-[0-9]{14,20}-[a-z0-9]{1,12}$/.test(id);
}

function profileKey(id) {
  return Object.keys(FRIENDLY_NAMES).find(prefix => id === prefix || id.startsWith(`${prefix}_`)
    || id.includes(`-${prefix}-`));
}
...
async function listDesignedVoices({ env = process.env, fetchImpl = global.fetch, timeoutMs = 15_000 } = {}) {
  ...
  return (data?.output?.voice_list || []).map(item => {
    ...
    if (!isViciVoiceID(id) || language !== 'en' || modelID !== TARGET_MODEL) return null;
    return {
      id,
...
async function designVoice({ preferredName, voicePrompt, previewText, env = process.env,
  ...
  if (!id || !isViciVoiceID(id) || !audio.length || audio.length > MAX_AUDIO_BYTES) {
...
async function speak({ text, voiceID, modelID = TARGET_MODEL, env = process.env,
  ...
  if (!isViciVoiceID(id) || modelID !== TARGET_MODEL) {
...
module.exports = {
  ...
  configuration, designVoice, downloadAudio, isViciVoiceID, listDesignedVoices, safeAudioURL, speak
};
