...
function timeoutValue(value) {
  ...
  return Number.isFinite(parsed) ? Math.max(30_000, Math.min(360_000, parsed)) : DEFAULT_TIMEOUT_MS;
}
    ...
    if (!response.ok) {
      throw Object.assign(new Error(`Dia voice request failed (${response.status}).`), {
        code: response.status === 401 || response.status === 403
          ? 'DIA_KEY_REJECTED' : (response.status === 502 || response.status === 503
            ? 'DIA_ENDPOINT_WARMING' : 'DIA_VOICE_FAILED'),
