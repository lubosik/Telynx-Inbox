...
const SYSTEM_PROMPT = [
  ...
  '',
  'SENDING, and be exact about this:',
  '- You cannot send anything yourself. request_campaign_send does not send. It',
  '  asks the owner to confirm with Face ID, and only their face sends it.',
  '- After calling it, say what is waiting for them: how many people, and how',
  '  many were suppressed if any were. Then stop and let them decide. Never say',
  '  a campaign has gone out, is going out, or is on its way.',
  '- If the tool refuses, say the reason. Nobody eligible is a real answer.',
  ...
  '- Never name a compound and never expand an abbreviation into the name it',
  '  stands for. The catalogue names them the short way for a reason.',
  '- You do not write the copy. The campaign writer does, under rules you do not',
  '  have. Do not offer to reword a message or suggest text for one.',
  '',
...
/**
 * What the model is told when a tool throws.
 *
 * Prefers the written sentence a typed error carries, because those are
 * deliberately phrased for a person: "Campaign opportunity proposals are
 * disabled. Set CAMPAIGN_OPPORTUNITY_PROPOSALS_ENABLED=true to enable them."
 * is something the assistant can say. `CAMPAIGN_PROPOSALS_DISABLED` is not.
 *
 * Falls back to the code, then to a generic line, and refuses anything that
 * looks like a raw database or network error rather than reading it out.
 */
function readableToolFailure(error) {
  const message = typeof error?.message === 'string' ? error.message.trim() : '';
  // No trailing \b after an alternative that ends in a space: "relation \b"
  // can never match, because a space followed by a quote is not a word
  // boundary. The first version of this let `relation "sms_x" does not exist`
  // straight through to be read out loud.
  const looksInternal = /\bPGRST|\bECONN|\bETIMEDOUT|\bENOTFOUND|fetch failed|\brelation\b|\bcolumn\b|syntax error|violates .* constraint/i
    .test(message);
  if (message && !looksInternal && message.length <= 200) return message;
  return error?.code || 'That did not succeed.';
}
...
async function converse({
  ...
  let navigate = null;
  // The last send the model asked the operator to confirm. Carried out for the
  // same reason navigation is: the server cannot scan a face, and this one is
  // load-bearing rather than convenient. If it did not reach the app, nothing
  // is sent, which is the correct way for this to fail.
    ...
    for (const call of completion.toolCalls) {
      ...
      if (!tool) {
        ...
        if (args === null) {
          ...
          // RECORDED BEFORE THE AWAIT, not after it.
          //
          // A tool that threw used to leave no trace: the push was below the
          // call, so the saved thread showed only the tools that had SUCCEEDED.
          // A session where drafting was attempted and failed was
          // indistinguishable afterwards from one where it was never tried,
          // which is exactly the wrong way round for diagnosing a complaint.
          toolsUsed.push(tool.name);
          try {
            result = await tool.run(args, { actor });
          } catch (error) {
            // The stack never leaves this process and neither does a raw
            // database error. But a bare code does not either, now: this is a
            // VOICE interface, and "CAMPAIGN_PROPOSALS_DISABLED" is not a
            // sentence anybody can say out loud. The typed errors in this
            // codebase carry a written message meant for a person; the code is
            // kept alongside it rather than instead of it.
            result = {
              ok: false,
              reason: 'tool_failed',
              detail: readableToolFailure(error),
      ...
      if (result?.confirmSend?.campaignId) confirmSend = result.confirmSend;
      messages.push({
        role: 'tool',
