...
const TRANSLATIONS = [
  {
    // A scale-to-zero GPU endpoint may need time to load the pinned Dia model.
    // The operator can fix this one without knowing what a 502 or model pod is.
    matches: error => error?.code === 'DIA_ENDPOINT_WARMING',
    sentence: () => 'The Dia voice is still warming up. Wait about a minute and try the preview again'
  },
  {
...
function presentError(error, { action = 'doing that', logger = console } = {}) {
  ...
  return {
    ...
    body: {
      ...
      code: translation
        ? (code === 'DIA_ENDPOINT_WARMING' ? 'VOICE_WARMING_UP' : 'PENDING_DATABASE_UPDATE')
        : 'INTERNAL_ERROR',
      reference
    }
  };
