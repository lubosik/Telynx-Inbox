...
function playAudioOnCall(callControlId, audioUrl, { loop = 1, commandId } = {}, options) {
  const body = {
    audio_url: audioUrl,
    loop,
    cache_audio: true
  ...
  if (commandId) body.command_id = commandId;
  return telnyxPost(`/calls/${encodeURIComponent(callControlId)}/actions/playback_start`, body, options);
}

function stopAudioOnCall(callControlId, options) {
  return telnyxPost(`/calls/${encodeURIComponent(callControlId)}/actions/playback_stop`, {
    stop: 'all'
  }, options);
}
