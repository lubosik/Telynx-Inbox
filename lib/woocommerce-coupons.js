...
function generateCode({ prefix = '', seed } = {}) {
  ...
  if (!CODE_SHAPE.test(code)) {
    // Unreachable by construction, but a code that merge-fields would drop
    // must never leave this function, so the invariant is checked anyway.
    throw new CouponRequestError(`Generated code failed shape check: ${code}`, 'COUPON_CODE_SHAPE_FAILED', 500);
  }
  return code;
}
 ...
 */
function couponSpec({ code, percent, usageLimit = 1, expiresAt, emailRestriction } = {}) {
  ...
  if (emailRestriction !== undefined && emailRestriction !== null) {
    const email = String(emailRestriction).trim().toLowerCase();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      throw new CouponRequestError('emailRestriction must be an email address.', 'COUPON_EMAIL_INVALID');
    }
