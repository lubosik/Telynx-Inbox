 ...
 */
/**
 * Update an existing coupon in place.
 *
 * A memorable code is REUSED across campaigns by design, and that is the
 * danger: a coupon still carrying the previous campaign's email restrictions
 * would refuse everybody in the new one, silently, at checkout. Deleting and
 * recreating would lose the usage history and let somebody redeem twice, so
 * the restrictions and the amount are updated on the row that already exists.
 */
async function updateCoupon(id, patch, opts = {}) {
  const { fetchImpl = global.fetch } = opts;
  const numericId = Number(id);
  if (!Number.isInteger(numericId) || numericId < 1) {
    throw new CouponRequestError('updateCoupon requires a positive integer id.', 'COUPON_ID_INVALID');
  }
  const response = await fetchImpl(`${WC_URL}/coupons/${numericId}`, {
    method: 'PUT',
    headers: { ...wooAuth(), 'Content-Type': 'application/json' },
