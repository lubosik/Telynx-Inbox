...
async function prepareVoiceAudio({ client, attemptID, scripts, voiceID, modelID,
  ...
  const [human, voicemail] = await Promise.all([
    synthesize({ text: scripts.human, voiceID, env: { ...env, ELEVENLABS_MODEL: modelID }, fetchImpl }),
    synthesize({ text: scripts.voicemail, voiceID, env: { ...env, ELEVENLABS_MODEL: modelID }, fetchImpl })
  ]);
