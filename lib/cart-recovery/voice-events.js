...
function createVoiceEventHandler({ client, env = process.env, api = {}, now = () => new Date(), schedule = null } = {}) {
  ...
  async function speak(ctx, branch, eventID) {
    ...
    try {
      ...
      // Every production call is pre-rendered before dialing. Never fall back
      // to live synthesis here: it could use changed settings and would make a
      // historical call's voice/provider non-deterministic.
      if (!audioURL) throw Object.assign(new Error('Staged recovery audio is unavailable.'), {
  ...
  function scheduleMachineSpeechFallback(ctx) {
    // A few carrier mailboxes classify early but never emit Telnyx's optional
    // greeting-ended webhook. Those mailboxes can disconnect after roughly
    // 15 seconds of post-greeting silence, so a 30-second fallback can never
    // deposit the message. Eight seconds leaves room for the greeting while
    // keeping playback inside the shortest mailbox recording window we have
    // observed in production.
    const configured = Number(env.VICI_VOICE_MACHINE_FALLBACK_MS || 8000);
