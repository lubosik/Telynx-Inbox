...
function createVoiceEventHandler({ client, env = process.env, api = {}, now = () => new Date() } = {}) {
  ...
  async function handle(event) {
    ...
    if (type === 'call.initiated') {
    ...
    } else if (type.includes('machine') && (type.includes('detection') || type.includes('premium'))) {
      if (TERMINAL_STATES.has(ctx.attempt.state) || SPEECH_STATES.has(ctx.attempt.state)
          || TRANSFER_STATES.has(ctx.attempt.state)) return { handled: true };
      ...
      if (branch === 'human') {
        if (!await transitionRecovery(ctx.recovery.id, ['ANSWERED'], { voice_status: 'HUMAN_DETECTED' })) {
          return { handled: true };
        }
        ...
        if (!await transitionRecovery(ctx.recovery.id, ['ANSWERED'], { voice_status: 'MACHINE_DETECTED' })) {
          return { handled: true };
        }
        await timeline(ctx.recovery.id, 'VOICE_MACHINE_DETECTED', at, { result });
        if (result.includes('beep')) await speak(ctx, 'machine', commandID());
      } else {
