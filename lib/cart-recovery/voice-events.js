...
function createVoiceEventHandler({ client, env = process.env, api = {}, now = () => new Date(), schedule = null } = {}) {
  ...
  async function speak(ctx, branch, eventID) {
    ...
    try {
      ...
      const audioURL = await signedVoiceAudioURL({
  ...
  async function startEarlyHumanAudio(ctx, at) {
    ...
    const audioURL = await signedVoiceAudioURL({ client, attemptID: ctx.attempt.id, branch: 'human' });
    if (!audioURL) return false;
    if (!await transitionAttempt(ctx.attempt.id, ['ANSWERED'], { state: 'HUMAN_MESSAGE_PLAYING' })) return false;
    if (!await transitionRecovery(ctx.recovery.id, ['ANSWERED'], { voice_status: 'HUMAN_MESSAGE_PLAYING' })) {
      await transitionAttempt(ctx.attempt.id, ['HUMAN_MESSAGE_PLAYING'], { state: 'ANSWERED' });
      return false;
    }
    await timeline(ctx.recovery.id, 'VOICE_EARLY_HUMAN_AUDIO', at, { attempt_id: ctx.attempt.id });
    try {
      ...
      await transitionAttempt(ctx.attempt.id, ['HUMAN_MESSAGE_PLAYING'], { state: 'ANSWERED' });
      await transitionRecovery(ctx.recovery.id, ['HUMAN_MESSAGE_PLAYING'], { voice_status: 'ANSWERED' });
      return false;
    }
  }
  ...
  async function handle(event) {
    ...
    if (type === 'call.initiated') {
      ...
      await startEarlyHumanAudio({
        ...ctx,
        attempt: { ...ctx.attempt, state: 'ANSWERED', answered_at: at },
        recovery: { ...ctx.recovery, voice_status: 'ANSWERED' }
      }, at);
