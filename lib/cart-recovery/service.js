...
const {
  ...
  insideCallingWindow, renderVoiceScripts, voiceConfiguration
} = require('./voice');
const { createOutboundCall, hangupCall } = require('../telnyx-api');
const { listAccountVoices } = require('../assistant/voice');
...
function createCartRecoveryService({
  ...
  async function updateSettings({ input, actor }) {
    ...
    if (requestedVoiceID) {
      const catalogue = await listAccountVoices({ env, fetchImpl });
    ...
    // Render with safe placeholders to validate mandatory disclosures before
    // any setting can be persisted.
    const wantsVoiceEnabled = source.voiceEnabled === true || source.voice_enabled === true;
    if (wantsVoiceEnabled) {
      renderVoiceScripts({ firstName: 'Alex', items: [{ product_name: 'RT' }], tollFreeNumber:
        source.voiceOptOutTollFreeNumber ?? source.voice_opt_out_toll_free_number ?? env.VICI_VOICE_OPT_OUT_TOLL_FREE_NUMBER,
  ...
  async function listRecoveryVoices() {
    const voices = await listAccountVoices({ env, fetchImpl });
    return { voices: voices.filter(voice => voice.verified === true),
      authorizationBoundary: 'connected_elevenlabs_account_verified_voices' };
  }
  ...
  async function runVoiceDue() {
    ...
    for (const cart of rows || []) {
      ...
      try {
        ...
        if (!productionAllowed && !testNumbers.has(cart.contact_phone)) {
          await rpc('defer_luko_cart_voice_call', { p_id: cart.id, p_claim: claim,
  ...
  return { processEvent, runDue, runPushDue, runVoiceDue, listRecoveryVoices, metrics, preflight, markDelivery, handleInboundReply, handleOptOut, reconcileWooOrder,
