...
const { cleanupVoiceAudio, prepareVoiceAudio } = require('./voice-audio-cache');
...
const { TARGET_MODEL: QWEN_TARGET_MODEL, listDesignedVoices, speak: speakQwen } = require('../assistant/qwen-voice');
const { listDiaVoices: listConfiguredDiaVoices, speak: speakDia } = require('../assistant/dia-voice');
...
function createCartRecoveryService({
  ...
  listQwenVoices = listDesignedVoices,
  synthesizeQwenSpeech = speakQwen,
  listDiaVoices = listConfiguredDiaVoices,
  synthesizeDiaSpeech = speakDia,
  stageVoiceAudio = prepareVoiceAudio
} = {}) {
  ...
  async function updateSettings({ input, actor }) {
    ...
    if (requestedVoiceID) {
      ...
      authorizedVoice = catalogue.voices.find(voice => voice.id === requestedVoiceID) || null;
      if (!authorizedVoice || authorizedVoice.verified !== true) {
        throw Object.assign(new Error('Choose a voice from the authorized Vin voice list.'), {
          status: 400, code: 'UNAUTHORIZED_RECOVERY_VOICE',
          publicMessage: 'That voice is no longer available. Choose another Vin voice and try again.'
        });
      }
    }
    ...
    const patch = {
      ...
      voice_provider: authorizedVoice?.provider || 'elevenlabs',
      voice_id: requestedVoiceID || null,
      voice_name: authorizedVoice?.name || null,
      voice_model_id: String(authorizedVoice?.modelId
        || (source.voiceModelId ?? source.voice_model_id) || 'eleven_turbo_v2_5').slice(0, 80),
  ...
  async function listRecoveryVoices() {
    ...
    if (env.ELEVENLABS_API_KEY) {
      providers.push({ name: 'ElevenLabs', promise: listVoices({ env, fetchImpl }).then(voices => curatedRecoveryVoices(voices).map(voice => ({
        ...voice, provider: voice.provider || 'elevenlabs', providerLabel: voice.providerLabel || 'ElevenLabs',
        modelId: voice.modelId || env.ELEVENLABS_MODEL || 'eleven_turbo_v2_5', syntheticDesign: false
      }))) });
    } else if (listVoices !== listAccountVoices) {
      // Tests and explicitly injected catalogues do not need production keys.
      providers.push({ name: 'ElevenLabs', promise: listVoices({ env, fetchImpl }).then(voices => curatedRecoveryVoices(voices).map(voice => ({
        ...voice, provider: voice.provider || 'elevenlabs', providerLabel: voice.providerLabel || 'ElevenLabs',
        modelId: voice.modelId || env.ELEVENLABS_MODEL || 'eleven_turbo_v2_5', syntheticDesign: false
      }))) });
    }
    // Qwen VoiceDesign was piloted, reviewed by the owner, and removed from
    // the production picker because the designed voices did not sound natural
    // enough. Keep provider-aware playback for immutable historical attempts,
    // but never surface those voices again unless a future review explicitly
    // opts the catalogue back in.
    if (env.QWEN_VOICE_CATALOGUE_ENABLED === 'true'
        && env.DASHSCOPE_API_KEY && env.DASHSCOPE_WORKSPACE_ID) {
      providers.push({ name: 'Qwen', promise: listQwenVoices({ env, fetchImpl }) });
    }
    if (env.DIA_VOICE_CATALOGUE_ENABLED === 'true'
        && env.DIA_TTS_ENDPOINT && env.DIA_TTS_API_KEY) {
      providers.push({ name: 'Dia', promise: listDiaVoices({ env, fetchImpl }) });
    }
    const settled = await Promise.allSettled(providers.map(provider => provider.promise));
    const voices = settled.filter(result => result.status === 'fulfilled').flatMap(result => result.value);
    if (!voices.length && settled.some(result => result.status === 'rejected')) throw settled.find(result => result.status === 'rejected').reason;
    return { voices,
      providerWarnings: settled.flatMap((result, index) => result.status === 'rejected'
        ? [`${providers[index].name} voices are temporarily unavailable.`] : []),
      authorizationBoundary: 'connected_provider_accounts_curated_authorized_us_voices' };
  }

  async function synthesizeAuthorizedVoice({ provider = 'elevenlabs', text: spoken, voiceID, modelID, synthesisEnv = env } = {}) {
    if (provider === 'qwen') {
      return synthesizeQwenSpeech({ text: spoken, voiceID, modelID: modelID || QWEN_TARGET_MODEL,
        env: synthesisEnv, fetchImpl });
    }
    if (provider === 'dia') {
      return synthesizeDiaSpeech({ text: spoken, voiceID, modelID,
        env: synthesisEnv, fetchImpl });
    }
    if (provider === 'elevenlabs') {
      return synthesizeSpeech({ text: spoken, voiceID,
        env: { ...synthesisEnv, ELEVENLABS_MODEL: modelID || synthesisEnv.ELEVENLABS_MODEL }, fetchImpl });
    }
    throw Object.assign(new Error('That recovery voice provider is not supported.'), {
      code: 'INVALID_RECOVERY_VOICE_PROVIDER'
    });
  }
  ...
  async function previewRecoveryVoice({ voiceID } = {}) {
    ...
    const result = await synthesizeAuthorizedVoice({ provider: voice.provider,
      text: RECOVERY_VOICE_PREVIEW_TEXT, voiceID: voice.id, modelID: voice.modelId });
    ...
    return {
      ...
      voice: { id: voice.id, name: voice.name, provider: voice.provider }
    };
  }
  ...
  async function previewRecoveryAttempt({ journeyId, branch } = {}) {
    ...
    const attempt = await one('luko_cart_voice_attempts',
      'id,voice_provider,voice_id,voice_model_id,rendered_human_text,rendered_voicemail_text,human_message_played_at,voicemail_played_at,dry_run',
    ...
    const voices = (await listRecoveryVoices()).voices;
    ...
    const result = await synthesizeAuthorizedVoice({ text: spoken,
      provider: attempt.voice_provider || 'elevenlabs', voiceID: attempt.voice_id,
      modelID: attempt.voice_model_id });
    if (!Buffer.isBuffer(result?.audio) || result.audio.length === 0) {
    ...
    for (const cart of rows || []) {
      ...
      try {
        ...
        const attempt = await rpc('begin_luko_cart_voice_call', { p_id: cart.id, p_claim: claim, p_attempt: {
          ...
          voice_provider: configuration.voiceProvider,
        ...
        try {
          const stagedAudio = await stageVoiceAudio({
            client, attemptID: attempt.attempt_id, scripts,
            provider: configuration.voiceProvider, voiceID: configuration.voiceID, modelID: configuration.modelID,
            synthesize: ({ text: spoken, provider, voiceID, modelID, env: synthesisEnv }) =>
              synthesizeAuthorizedVoice({ text: spoken, provider, voiceID, modelID, synthesisEnv }),
            env, fetchImpl
          });
          if (stagedAudio?.ready !== true) {
            throw Object.assign(new Error('Recovery voice audio could not be staged.'), {
              code: 'VOICE_AUDIO_STAGING_UNAVAILABLE'
            });
          }
        } catch (error) {
          // Fail before dialing. Falling back to live synthesis can silently
          // use a different voice after settings change, and cannot preserve a
          // future self-hosted provider choice. A call without immutable audio
          // is less safe than a deferred recovery touch.
          console.warn('[CART VOICE] Pre-generated audio unavailable; call not placed:',
            error.code || error.message);
          await cleanupVoiceAudio({ client, attemptID: attempt.attempt_id }).catch(() => {});
          const at = now().toISOString();
          const { error: attemptError } = await client.from('luko_cart_voice_attempts').update({
            state: 'FAILED', failure_code: 'voice_audio_staging_failed', completed_at: at, updated_at: at
          }).eq('id', attempt.attempt_id);
          if (attemptError) throw attemptError;
          await rpc('defer_luko_cart_voice_call', { p_id: cart.id, p_claim: claim,
            p_status: 'FAILED', p_reason: 'voice_audio_staging_failed', p_due_at: null });
          summary.uncertain++;
          continue;
        }
        let accepted;
