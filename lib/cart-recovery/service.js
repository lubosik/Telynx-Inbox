...
function createCartRecoveryService({
  ...
  function serializeJourney(row, secrets = {}) {
    ...
    return {
      ...
      orderId: row?.order_id || null, recoveredRevenue: recovered ? (row?.order_total || 0) : null,
  ...
  function serializeTimeline(row) {
    ...
    return { id: String(row?.id || ''), type, title: type.replaceAll('_', ' ').toLowerCase().replace(/\b\w/g, letter => letter.toUpperCase()),
      detail: row?.metadata?.reason || row?.metadata?.primary_category || null, createdAt: row?.occurred_at || row?.created_at || null };
  }
  ...
  async function getJourney({ id }) {
    ...
    const [replyResult, timelineResult] = await Promise.all([
    ...
    if (replyResult.error || timelineResult.error) throw Object.assign(new Error('Recovery journey details are unavailable.'), { code: replyResult.error?.code || timelineResult.error?.code || 'CART_DATABASE_ERROR' });
    ...
    return {
      journey: serializeJourney(journey, { message: decrypt(journey.message_ciphertext), recoveryURL: decrypt(journey.recovery_ciphertext) }),
  ...
  return { processEvent, runDue, runPushDue, metrics, preflight, markDelivery, handleInboundReply, handleOptOut,
