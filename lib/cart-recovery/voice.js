...
function voiceConfiguration(env = process.env, settings = {}) {
  ...
  const transferNumber = String(settings.voice_transfer_number || env.VICI_LIVE_TRANSFER_NUMBER || '').trim();
  ...
  if (transferNumber && !/^\+[1-9]\d{7,14}$/.test(transferNumber)) errors.push('invalid_transfer_number');
  if (transferNumber && transferNumber === env.TELNYX_PHONE_NUMBER) errors.push('transfer_loop');
