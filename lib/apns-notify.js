 ...
 *
 * Six kinds of push live here and they are deliberately NOT the same payload:
 *
 ...
 *   sendReferralNotifications — an internal handoff to named, owned devices.
 *                             Carries a conversation route but no badge.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 * goes to Apple. Not at the call sites that decide to notify. There are six
 ...
 * DECIDE TO NOTIFY. There are six senders in this file and there will be more;
...
async function sendNativeMessagePush({ title, body, phone, isVIP = false }, options = {}) {
  ...
  const basePayload = {
    aps: {
      alert: { title, body },
      sound: 'default',
      'thread-id': isVIP ? `vip-${phone || 'vici-inbox'}` : (phone || 'vici-inbox'),
      ...(isVIP ? { category: 'VIP_MESSAGE' } : {})
    },
    phone: phone || '',
    ...(isVIP ? { vip: true } : {})
  };
...
function referralPayload(notification) {
  if (notification?.channel !== 'native_push_preparation') return null;
  if (!['referrals.assigned', 'referrals.resolved'].includes(notification?.eventType)) return null;
  const title = String(notification?.payload?.aps?.alert?.title || '').trim().slice(0, 120);
  const body = String(notification?.payload?.aps?.alert?.body || '').trim().slice(0, 500);
  const referralID = String(notification?.payload?.referralID || '').trim().slice(0, 128);
  const phone = String(notification?.payload?.phone || '').trim();
  if (!title || !body || !referralID || !/^[+][1-9][0-9]{7,14}$/.test(phone)) return null;
  return {
    aps: {
      alert: { title, body }, sound: 'default',
      category: 'REFERRAL',
      'thread-id': `vici-referral-${referralID}`.slice(0, 64)
    },
    screen: 'conversation',
    referralID,
    phone
...
/**
 * Referral alerts contain internal work assignment data, so compatibility
 * devices without a named user owner are never eligible. Real delivery is off
 * until REFERRAL_NOTIFICATIONS_ENABLED is exactly `true`; dry runs may still
 * prove targeting and preference filtering safely.
 */
async function sendReferralNotifications(notifications = [], { dryRun = true } = {}, options = {}) {
  const env = options.env || process.env;
  const prepared = (notifications || []).filter(row =>
    ['referrals.assigned', 'referrals.resolved'].includes(row?.eventType));
  const payload = referralPayload(prepared[0]);
  if (!payload || prepared.some(row => JSON.stringify(referralPayload(row)) !== JSON.stringify(payload))) {
    return { sent: 0, targeted: 0, error: 'invalid_referral_notification_preparation' };
  }
  const userIDs = new Set(prepared.map(row => normaliseUserID(row.userID)).filter(Boolean));
  if (!userIDs.size) return { sent: 0, targeted: 0, error: 'referral_notification_targets_missing' };
  if (!dryRun && env.REFERRAL_NOTIFICATIONS_ENABLED !== 'true') {
  ...
  if (error) return { sent: 0, targeted: 0, error: 'referral_notification_device_load_failed' };
  const owned = devices.filter(row => row.user_id && userIDs.has(normaliseUserID(row.user_id)));
  const wanted = await splitByPreference(owned, 'referrals', client);
  ...
  const config = configuration();
  if (!config && !options.authorization) return reportMissingConfiguration('referral notifications');
  let authorization = options.authorization;
  if (!authorization) {
    try { authorization = providerToken(config); } catch (tokenError) {
      console.error('APNs: referral provider token creation failed:', tokenError.message);
      return { sent: 0, targeted: targets.length, error: 'referral_notification_token_failed' };
    }
  }
  const deliverNotifications = options.deliver || deliver;
  const result = await deliverNotifications(
    targets, payload,
    { 'apns-collapse-id': collapseIdentifier(prepared[0].collapseID || `vici-referral-${payload.referralID}`) },
    { ...options, client, authorization }
  );
