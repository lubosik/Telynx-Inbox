-- LUKO / Vici Nari Labs Dia recovery-voice provider.
-- Additive and rerunnable. Apply AFTER qwen-voice-provider-migration.sql and
-- BEFORE selecting a Dia profile. This does not enable automation, choose a
-- voice, place a call, or alter consent/suppression evidence.
...
BEGIN
  IF to_regclass('public.luko_cart_recovery_settings') IS NULL
    ...
    RAISE EXCEPTION 'voice recovery migrations must be applied first';
  END IF;
END
$$;

ALTER TABLE public.luko_cart_recovery_settings
  DROP CONSTRAINT IF EXISTS luko_cart_recovery_settings_voice_provider_check;
ALTER TABLE public.luko_cart_recovery_settings
...
ALTER TABLE public.luko_cart_voice_attempts
  DROP CONSTRAINT IF EXISTS luko_cart_voice_attempts_voice_provider_check;
ALTER TABLE public.luko_cart_voice_attempts
